...
````bash
jellog add-package DataGap.Jellog.TextTemplating.Scriban
````
...
1. Add the [DataGap.Jellog.TextTemplating.Scriban](https://www.nuget.org/packages/DataGap.Jellog.TextTemplating.Scriban) NuGet package to your project:

````
Install-Package DataGap.Jellog.TextTemplating.Scriban
````

2. Add the `JellogTextTemplatingScribanModule` to the dependency list of your module:

````csharp
[DependsOn(
    //...other dependencies
    typeof(JellogTextTemplatingScribanModule) //Add the new module dependency
    )]
...
````xml
<ItemGroup>
  <None Remove="DataGap\Jellog\Emailing\Templates\*.tpl" />
  <EmbeddedResource Include="DataGap\Jellog\Emailing\Templates\*.tpl" />
</ItemGroup>
````
