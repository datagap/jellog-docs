...
* `AsyncPeriodicBackgroundWorkerBase` uses the `JellogTimer` (a thread-safe timer) object to determine **the period**. We can set its `Period` property in the constructor.
...
`JellogBackgroundWorkerOptions` class is used to [set options](Options.md) for the background workers. Currently, there is only one option:
