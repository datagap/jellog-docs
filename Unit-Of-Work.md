...
JELLOG Framework's Unit Of Work (UOW) implementation provides an abstraction and control on a **database connection and transaction** scope in an application.
...
All of these are automatically handled by the JELLOG Framework.
...
`JellogUnitOfWorkDefaultOptions` is used to configure the default options for the unit of work system. Configure the options in the `ConfigureServices` method of your [module](Module-Development-Basics.md).
...
namespace JellogDemo.Web
{
    public class MyController : JellogController
    {
...
* `Complete` and `Rollback` methods are used to complete (commit) or roll backs the current UOW, which are normally used internally by the JELLOG Framework but can be used if you manually start a transaction using the `IUnitOfWorkManager.Begin` method.
...
`JellogUnitOfWorkMiddleware` is a middleware that can enable UOW in the ASP.NET Core request pipeline. This might be needed if you need to enlarge the UOW scope to cover some other middleware(s).
