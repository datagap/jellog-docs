...
We prefer to **delete the database** {{if DB=="EF"}}(you can run the `Drop-Database` in the *Package Manager Console*){{end}} since this is just an example project and data loss is not important. Since this topic is not related to the JELLOG Framework, we don't go deeper for all the scenarios.
...
{ {{if DB=="Mongo"}}
    ...
    public class BookAppService_Tests : BookStoreApplicationTestBase
    {
        ...
        {
            var exception = await Assert.ThrowsAsync<JellogValidationException>(async () =>
            {
...
These changes require a small change in the `EditModal.cshtml`. Remove the `<jellog-input asp-for="Id" />` tag since we no longer need to it (since moved it to the `EditBookViewModel`). The final content of the `EditModal.cshtml` should be following:
...
<ngx-datatable-column
  [name]="'::Author' | jellogLocalization"
  prop="authorName"
  [sortable]="false"
></ngx-datatable-column>
````
...
export class BookComponent implements OnInit {
  ...
  delete(id: string) {
    this.confirmation.warn('::AreYouSureToDelete', 'JellogAccount::AreYouSure').subscribe((status) => {
...
* It is essential to call the `base.OnInitializedAsync()` since `JellogCrudPageBase` has some initialization code to be executed.
