...
@section scripts
{
    <jellog-script src="/Pages/Authors/Index.js"/>
}
...
<jellog-card>
    <jellog-card-header>
        <jellog-row>
            <jellog-column size-md="_6">
                <jellog-card-title>@L["Authors"]</jellog-card-title>
            </jellog-column>
            <jellog-column size-md="_6" class="text-end">
                @if (await AuthorizationService
                    .IsGrantedAsync(BookStorePermissions.Authors.Create))
                {
                    <jellog-button id="NewAuthorButton"
    ...
    <jellog-card-body>
        <jellog-table striped-rows="true" id="AuthorsTable"></jellog-table>
    </jellog-card-body>
</jellog-card>
````
...
$(function () {
    ...
    var createModal = new jellog.ModalManager(jellog.appPath + 'Authors/CreateModal');
    var editModal = new jellog.ModalManager(jellog.appPath + 'Authors/EditModal');
    ...
    var dataTable = $('#AuthorsTable').DataTable(
        jellog.libs.datatables.normalizeConfiguration({
            ...
            searching: false,
            scrollX: true,
            ajax: jellog.libs.datatables.createAjax(acme.bookStore.authors.author.getList),
            columnDefs: [
                {
                    title: l('Actions'),
                    rowAction: {
                            ...
                            [
                                {
                                    text: l('Edit'),
                                    visible: 
                                        jellog.auth.isGranted('BookStore.Authors.Edit'),
                                ...
                                },
                                {
                                    text: l('Delete'),
                                    visible: 
                                        jellog.auth.isGranted('BookStore.Authors.Delete'),
...
* Uses the `jellog.ModalManager` to open *Create* and *Edit* modal forms.
...
<form asp-page="/Authors/CreateModal">
    <jellog-modal>
        <jellog-modal-header title="@L["NewAuthor"].Value"></jellog-modal-header>
        <jellog-modal-body>
            <jellog-input asp-for="Author.Name" />
            <jellog-input asp-for="Author.BirthDate" />
            <jellog-input asp-for="Author.ShortBio" />
        </jellog-modal-body>
...
We had used [dynamic forms](../UI/AspNetCore/Tag-Helpers/Dynamic-Forms.md) of the JELLOG Framework for the books page before. We could use the same approach here, but we wanted to show how to do it manually. Actually, not so manually, because we've used `jellog-input` tag helper in this case to simplify creating the form elements.

You can definitely use the standard Bootstrap HTML structure, but it requires to write a lot of code. `jellog-input` automatically adds validation, localization and other standard elements based on the data type.
...
<form asp-page="/Authors/EditModal">
    <jellog-modal>
        ...
        <jellog-modal-body>
            <jellog-input asp-for="Author.Id" />
            <jellog-input asp-for="Author.Name" />
            <jellog-input asp-for="Author.BirthDate" />
            <jellog-input asp-for="Author.ShortBio" />
        </jellog-modal-body>
...
[JELLOG CLI](https://docs.jellog.io/en/jellog/latest/CLI) provides `generate-proxy` command that generates client proxies for your HTTP APIs to make easy to consume your HTTP APIs from the client side. Before running `generate-proxy` command, your host must be up and running.
...
<div class="card">
  <div class="card-header">
    <div class="row">
      <div class="col col-md-6">
        <h5 class="card-title">
          {%{{{ '::Menu:Authors' | jellogLocalization }}}%}
        </h5>
      </div>
      <div class="text-end col col-md-6">
        <div class="text-lg-end pt-2">
          <button *jellogPermission="'BookStore.Authors.Create'" id="create" class="btn btn-primary" type="button" (click)="createAuthor()">
  ...
  <div class="card-body">
    <ngx-datatable [rows]="author.items" [count]="author.totalCount" [list]="list" default>
      ...
      >
        <ng-template let-row="row" ngx-datatable-cell-template>
          <div ngbDropdown container="body" class="d-inline-block">
            ...
            <div ngbDropdownMenu>
              <button *jellogPermission="'BookStore.Authors.Edit'" ngbDropdownItem (click)="editAuthor(row.id)">
                {%{{{ '::Edit' | jellogLocalization }}}%}
              </button>
              <button *jellogPermission="'BookStore.Authors.Delete'" ngbDropdownItem (click)="delete(row.id)">
      ...
      <ngx-datatable-column [name]="'::BirthDate' | jellogLocalization">
        <ng-template let-row="row" ngx-datatable-cell-template>
          {%{{{ row.birthDate | date }}}%}
        </ng-template>
      </ngx-datatable-column>
...
<jellog-modal [(visible)]="isModalOpen">
  <ng-template #jellogHeader>
    <h3>{%{{{ (selectedAuthor.id ? '::Edit' : '::NewAuthor') | jellogLocalization }}}%}</h3>
  </ng-template>
...
* This code is similar to the `Books.razor`, except it doesn't inherit from the `JellogCrudPageBase`, but uses its own implementation.
