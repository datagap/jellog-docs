...
It's common to call the HTTP API endpoints via AJAX from the **JavaScript** side. You can use `$.ajax` or another tool to call the endpoints. However, JELLOG offers a better way.

JELLOG **dynamically** creates **[JavaScript Proxies](../UI/AspNetCore/Dynamic-JavaScript-Proxies.md)** for all the API endpoints. So, you can use any **endpoint** just like calling a **JavaScript function**.
...
* Localization key names are arbitrary. You can set any name. We prefer some conventions for specific text types;
  ...
  * Use `Enum:<enum-type>:<enum-value>` naming convention to localize the enum members. When you do it like that, JELLOG can automatically localize the enums in some proper cases.
...
> JELLOG's localization system is built on the [ASP.NET Core's standard localization](https://docs.microsoft.com/en-us/aspnet/core/fundamentals/localization) system and extends it in many ways. Check the [localization document](../Localization.md) for details.
...
* `jellog-script` [tag helper](https://docs.microsoft.com/en-us/aspnet/core/mvc/views/tag-helpers/intro) is used to add external **scripts** to the page. It has many additional features compared to the standard `script` tag. It handles **minification** and **versioning**. Check the [bundling & minification document](../UI/AspNetCore/Bundling-Minification.md) for details.
* `jellog-card` is a tag helper for Twitter Bootstrap's [card component](https://getbootstrap.com/docs/4.5/components/card/). There are other useful tag helpers provided by the JELLOG Framework to easily use most of  [bootstrap](https://getbootstrap.com/)'s components. You could use the regular HTML tags instead of these tag helpers, but using tag helpers reduces HTML code and prevents errors by the help of the IntelliSense and compiles time type checking. For further information, check the [tag helpers](../UI/AspNetCore/Tag-Helpers/Index.md) document.
...
* `jellog.localization.getResource` gets a function that is used to localize text using the same JSON file defined on the server side. In this way, you can share the localization values with the client side.
* `jellog.libs.datatables.normalizeConfiguration` is a helper function defined by the JELLOG Framework. There's no requirement to use it, but it simplifies the [Datatables](https://datatables.net/) configuration by providing conventional default values for missing options.
* `jellog.libs.datatables.createAjax` is another helper function to adapt the JELLOG's dynamic JavaScript API proxies to the [Datatable](https://datatables.net/)'s expected parameter format
...
> Notice: This tutorial is based on the JELLOG Framework v3.1.0+ If your project version is older, then please upgrade your solution. Check the [migration guide](../UI/Angular/Migration-Guide-v3.md) if you are upgrading an existing project with v2.x.
...
`RoutesService` is a service provided by the JELLOG Framework to configure the main menu and the routes.
...
[JELLOG CLI](../CLI.md) provides a `generate-proxy` command that generates client proxies for your HTTP APIs to make your HTTP APIs easy to consume by the client side. Before running the `generate-proxy` command, your host must be up and running.
...
* We are using the [ListService](../UI/Angular/List-Service.md), a utility service from the JELLOG Framework which provides easy pagination, sorting and searching.
...
<div class="card">
  ...
  <div class="card-body">
    <ngx-datatable [rows]="book.items" [count]="book.totalCount" [list]="list" default>
      <ngx-datatable-column [name]="'::Name' | jellogLocalization" prop="name"></ngx-datatable-column>
      <ngx-datatable-column [name]="'::Type' | jellogLocalization" prop="type">
        <ng-template let-row="row" ngx-datatable-cell-template>
          {%{{{ '::Enum:BookType:' + row.type | jellogLocalization }}}%}
        </ng-template>
      </ngx-datatable-column>
      <ngx-datatable-column [name]="'::PublishDate' | jellogLocalization" prop="publishDate">
        <ng-template let-row="row" ngx-datatable-cell-template>
          {%{{{ row.publishDate | date }}}%}
        </ng-template>
      </ngx-datatable-column>
      <ngx-datatable-column [name]="'::Price' | jellogLocalization" prop="price">
        <ng-template let-row="row" ngx-datatable-cell-template>
          {%{{{ row.price | currency }}}%}
        </ng-template>
      </ngx-datatable-column>
...
JELLOG Framework provides a generic base class - `JellogCrudPageBase<...>`, to create CRUD style pages. This base class is compatible with the `ICrudAppService` that was used to build the `IBookAppService`. So, we can inherit from the `JellogCrudPageBase` to automate the code behind for the standard CRUD stuff.
...
@using Microsoft.Extensions.Localization
@inject IStringLocalizer<BookStoreResource> L
@inherits JellogCrudPageBase<IBookAppService, BookDto, Guid, PagedAndSortedResultRequestDto, CreateUpdateBookDto>
...
* Inherited from  `JellogCrudPageBase<IBookAppService, BookDto, Guid, PagedAndSortedResultRequestDto, CreateUpdateBookDto>` which implements all the CRUD details for us.
...
#### About the JellogCrudPageBase

We will continue benefitting from  `JellogCrudPageBase` for the books page. You could just inject the `IBookAppService` and perform all the server side calls yourself (thanks to the [Dynamic C# HTTP API Client Proxy](../API/Dynamic-CSharp-API-Clients.md) system of the JELLOG Framework). We will do it manually for the authors page to demonstrate how to call the server side HTTP APIs in your Blazor applications.
