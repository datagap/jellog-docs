...
This is a hierarchical way of defining permission names. For example, "create book" permission name was defined as `BookStore.Books.Create`. JELLOG doesn't force you to a structure, but we find this way useful.
...
{
    text: l('Edit'),
    visible: jellog.auth.isGranted('BookStore.Books.Edit'), //CHECK for the PERMISSION
...
* `jellog.auth.isGranted(...)` is used to check a permission that is defined before.
...
import { AuthGuard, PermissionGuard } from '@jellog/ng.core';
...
* Imported `AuthGuard` and `PermissionGuard` from the `@jellog/ng.core`.
...
````html
<!-- Add the jellogPermission directive -->
<button *jellogPermission="'BookStore.Books.Create'" id="create" class="btn btn-primary" type="button" (click)="createBook()">
  <i class="fa fa-plus mr-1"></i>
  <span>{%{{{ '::NewBook' | jellogLocalization }}}%}</span>
</button>
````

* Just added `*jellogPermission="'BookStore.Books.Create'"` that hides the button if the current user has no permission.
...
````html
<!-- Add the jellogPermission directive -->
<button *jellogPermission="'BookStore.Books.Edit'" ngbDropdownItem (click)="editBook(row.id)">
  {%{{{ '::Edit' | jellogLocalization }}}%}
</button>

<!-- Add the jellogPermission directive -->
<button *jellogPermission="'BookStore.Books.Delete'" ngbDropdownItem (click)="delete(row.id)">
  {%{{{ '::Delete' | jellogLocalization }}}%}
</button>
````

* Added `*jellogPermission="'BookStore.Books.Edit'"` that hides the edit action if the current user has no editing permission.
* Added `*jellogPermission="'BookStore.Books.Delete'"` that hides the delete action if the current user has no delete permission.
...
The base `JellogCrudPageBase` class already has the necessary functionality for these kind of operations.
...
The base `JellogCrudPageBase` class automatically checks these permissions on the related operations. It also defines the given properties for us if we need to check them manually:
...
**JELLOG Framework caches the permissions** of the current user in the client side. So, when you change a permission for yourself, you need to manually **refresh the page** to take the effect. If you don't refresh and try to use the prohibited action you get an HTTP 403 (forbidden) response from the server.
