...
<jellog-dynamic-form jellog-model="Book" asp-page="/Books/CreateModal">
    <jellog-modal>
        <jellog-modal-header title="@L["NewBook"].Value"></jellog-modal-header>
        <jellog-modal-body>
            <jellog-form-content />
        </jellog-modal-body>
...
* This modal uses `jellog-dynamic-form` [tag helper](../UI/AspNetCore/Tag-Helpers/Dynamic-Forms.md) to automatically create the form from the `CreateUpdateBookDto` model class.
* `jellog-model` attribute indicates the model object where it's the `Book` property in this case.
* `jellog-form-content` tag helper is a placeholder to render the form controls (it is optional and needed only if you have added some other content in the `jellog-dynamic-form` tag, just like in this page).
...
Open the `Pages/Books/Index.cshtml` and set the content of `jellog-card-header` tag as below:

````html
<jellog-card-header>
    <jellog-row>
        <jellog-column size-md="_6">
            <jellog-card-title>@L["Books"]</jellog-card-title>
        </jellog-column>
...
* `jellog.ModalManager` is a helper class to manage modals on the client side. It internally uses Twitter Bootstrap's standard modal, but abstracts many details by providing a simple API.
...
<jellog-dynamic-form jellog-model="Book" asp-page="/Books/EditModal">
    <jellog-modal>
        <jellog-modal-header title="@L["Update"].Value"></jellog-modal-header>
        <jellog-modal-body>
...
* It includes an `jellog-input` for the `Id` property to store the `Id` of the editing book (which is a hidden input).
...
````js
{
    ...
    action: function (data) {
        acme.bookStore.books.book
            ...
            .then(function() {
                jellog.notify.info(l('SuccessfullyDeleted'));
                dataTable.ajax.reload();
            });
    }
}
...
* `jellog.notify.info()` shows a notification after the delete operation.
  ...
  <div class="card-header">
    <div class="row">
      <div class="col col-md-6">
        <h5 class="card-title">{%{{{ '::Menu:Books' | jellogLocalization }}}%}</h5>
      </div>        
...
<!-- Add the modal here -->
<jellog-modal [(visible)]="isModalOpen">
  <ng-template #jellogHeader>
    <h3>{%{{{ '::NewBook' | jellogLocalization }}}%}</h3>
  </ng-template>

  <ng-template #jellogBody> </ng-template>

  <ng-template #jellogFooter>
    <button type="button" class="btn btn-secondary" jellogClose>
      {%{{{ '::Close' | jellogLocalization }}}%}
    </button>
  </ng-template>
</jellog-modal>
```
...
* Added the `jellog-modal` which renders a modal to allow user to create a new book. `jellog-modal` is a pre-built component to show modals. While you could use another approach to show a modal, `jellog-modal` provides additional benefits.
...
Open `/src/app/book/book.component.html` and replace `<ng-template #jellogBody> </ng-template>`  with the following code part:
...
Also replace `<ng-template #jellogFooter> </ng-template>` with the following code part:
...
<ngx-datatable-column
  [name]="'::Actions' | jellogLocalization"
  [maxWidth]="150"
  [sortable]="false"
>
  <ng-template let-row="row" ngx-datatable-cell-template>
    <div ngbDropdown container="body" class="d-inline-block">
      <button
        ...
        ngbDropdownToggle
      >
        <i class="fa fa-cog mr-1"></i>{%{{{ '::Actions' | jellogLocalization }}}%}
      </button>
      <div ngbDropdownMenu>
...
Also, change the `ng-template #jellogHeader` section as shown below:

```html
<ng-template #jellogHeader>
    <h3>{%{{{ (selectedBook.id ? '::Edit' : '::NewBook' ) | jellogLocalization }}}%}</h3>
</ng-template>
```
...
// add new imports
import { ConfirmationService, Confirmation } from '@jellog/ng.theme.shared';
...
In this section, you will learn how to create a new modal dialog form to create a new book. Since we've inherited from the `JellogCrudPageBase`, we only need to develop the view part.
...
This code requires a service; Inject the `JellogBlazorMessageLocalizerHelper<T>` at the top of the file, just before the `@inherits...` line:

````csharp
@inject JellogBlazorMessageLocalizerHelper<BookStoreResource> LH
````

* The form implements validation and the `JellogBlazorMessageLocalizerHelper` is used to simply localize the validation messages.
...
The base `JellogCrudPageBase` uses the [object to object mapping](../Object-To-Object-Mapping.md) system to convert an incoming `BookDto` object to a `CreateUpdateBookDto` object. So, we need to define the mapping.
...
@using Acme.BookStore.Localization
@using Microsoft.Extensions.Localization
@using DataGap.Jellog.AspNetCore.Components.Web
@inject IStringLocalizer<BookStoreResource> L
@inject JellogBlazorMessageLocalizerHelper<BookStoreResource> LH
