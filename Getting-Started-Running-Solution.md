...
>
> The solution is configured to use **Entity Framework Core** with **MS SQL Server** by default. However, if you've selected another DBMS using the `-dbms` parameter on the JELLOG CLI `new` command (like `-dbms MySQL`), the connection string might be different for you.
>
...
> Before starting the application, run `jellog install-libs` command in your Web directory to restore the client-side libraries. This will populate the `libs` folder.
