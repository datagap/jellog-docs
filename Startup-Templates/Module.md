...
First, install the JELLOG CLI if you haven't installed before:
...
- It is fully integrated to JELLOG framework and all services in your application.
...
![Folder structure of JELLOG Angular module project](../images/angular-module-folder-structure.png)
...
The dependencies will be installed and some of them are JELLOG modules published as NPM packages. To see all JELLOG packages, you can run the following command in the `angular` folder:
...
The module you will develop depends on two of these JELLOG packages: _@jellog/ng.core_ and _@jellog/ng.theme.shared_. Rest of the JELLOG modules are included in _package.json_ because of the `dev-app` project.
...
![JELLOG Angular module dev-app project](../images/angular-module-dev-app-project.png)
...
- Essential JELLOG modules, i.e. `CoreModule` and `ThemeSharedModule`, are imported.
- `IssueManagementRoutingModule` is imported.
- `IssueManagementComponent` is declared.
- It is prepared for configurability. The `forLazy` static method enables [a configuration to be passed to the module when it is loaded by the router](https://dataGapsoft.com/blog/how-to-configure-angular-modules-loaded-by-the-router).
