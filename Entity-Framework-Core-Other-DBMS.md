...
[JELLOG CLI](CLI.md) provides a `-dbms` option to allow you to choose your Database Management System (DBMS) while creating a new solution. It accepts the following values:
...
So, if you want to use MySQL for your solution, you can use the `-dbms MySQL` option while using the `jellog new` command. Example:

````bash
jellog new BookStore -dbms MySQL
````

Also, the [Get Started page](https://jellog.io/get-started) on the JELLOG website allows you to select one of the providers.
...
* Remove the [DataGap.Jellog.EntityFrameworkCore.SqlServer](https://www.nuget.org/packages/DataGap.Jellog.EntityFrameworkCore.SqlServer) NuGet package dependency from the `.EntityFrameworkCore` project.
...
Remove the `JellogEntityFrameworkCoreSqlServerModule` from the dependency list of your ***YourProjectName*EntityFrameworkCoreModule** class.
...
* This code first checks if there is an existing (active) connection to the same database in the current request and reuses it if possible. This allows to share a single transaction among different DbContext types. JELLOG handles the rest of the things.
* It uses `ctx.ConnectionString` and passes to the `UseMySql` if there is no active connection (which will cause to create a new database connection). Using the `ctx.ConnectionString` is important here. Don't pass a static connection string (or a connection string from a configuration). Because JELLOG [dynamically determines the correct connection string](Connection-Strings.md) in a multi-database or [multi-tenant](Multi-Tenancy.md) environment.
...
Related discussions: https://github.com/jellogframework/jellog/issues/1920