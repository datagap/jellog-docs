...
* **Do** inherit the `DbContext` from the `JellogDbContext<TDbContext>` class.
...
````C#
[ConnectionStringName("JellogIdentity")]
public class IdentityDbContext : JellogDbContext<IdentityDbContext>, IIdentityDbContext
{
...
````C#
public static string TablePrefix { get; set; } = JellogIdentityConsts.DefaultDbTablePrefix;
public static string Schema { get; set; } = JellogIdentityConsts.DefaultDbSchema;
````

  - **Do** always use a short `TablePrefix` value for a module to create **unique table names** in a shared database. `Jellog` table prefix is reserved for JELLOG core modules.
...
public static class IdentityDbContextModelBuilderExtensions
{
    public static void ConfigureIdentity([NotNull] this ModelBuilder builder)
    {
        ...
        {
            b.ToTable(JellogIdentityDbProperties.DbTablePrefix + "Users", JellogIdentityDbProperties.DbSchema);
        ...
        {
            b.ToTable(JellogIdentityDbProperties.DbTablePrefix + "UserClaims", JellogIdentityDbProperties.DbSchema);
            b.ConfigureByConvention();
            //code omitted for brevity
        });
...
- **Do** add `DbContext` to the `IServiceCollection` using the `AddJellogDbContext<TDbContext>` method.
- **Do** add implemented repositories to the options for the `AddJellogDbContext<TDbContext>` method. Example:
...
public class JellogIdentityEntityFrameworkCoreModule : JellogModule
{
    public override void ConfigureServices(ServiceConfigurationContext context)
    {
        context.Services.AddJellogDbContext<IdentityDbContext>(options =>
        {
