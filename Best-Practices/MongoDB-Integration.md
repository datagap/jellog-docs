...
- **Do** define an **interface** for the `MongoDbContext` that inherits from `IJellogMongoDbContext`.
...
````C#
[ConnectionStringName("JellogIdentity")]
public interface IJellogIdentityMongoDbContext : IJellogMongoDbContext
{
...
- **Do** inherit the `MongoDbContext` from the `JellogMongoDbContext` class.
...
```c#
[ConnectionStringName("JellogIdentity")]
public class JellogIdentityMongoDbContext : JellogMongoDbContext, IJellogIdentityMongoDbContext
{
...
```c#
public static string CollectionPrefix { get; set; } = JellogIdentityConsts.DefaultDbTablePrefix;
```

Used the same constant defined for the EF Core integration table prefix in this example.

- **Do** always use a short `CollectionPrefix` value for a module to create **unique collection names** in a shared database. `Jellog` collection prefix is reserved for JELLOG core modules.
...
```c#
public static class JellogIdentityMongoDbContextExtensions
{
        ...
        Action<IdentityMongoModelBuilderConfigurationOptions> optionsAction = null)
    {
        ...
        {
            b.CollectionName = JellogIdentityDbProperties.DbTablePrefix + "Users";
        });

        builder.Entity<IdentityRole>(b =>
        {
            b.CollectionName = JellogIdentityDbProperties.DbTablePrefix + "Roles";
        });
    }
}
```
...
public class MongoIdentityUserRepository
    : MongoDbRepository<IJellogIdentityMongoDbContext, IdentityUser, Guid>,
      IIdentityUserRepository
{
    public MongoIdentityUserRepository(
        IMongoDbContextProvider<IJellogIdentityMongoDbContext> dbContextProvider) 
...
[DependsOn(
    ...
    typeof(JellogUsersMongoDbModule)
    )]
public class JellogIdentityMongoDbModule : JellogModule
{
    public override void ConfigureServices(ServiceConfigurationContext context)
    {
        context.Services.AddMongoDbContext<JellogIdentityMongoDbContext>(options =>
        {
