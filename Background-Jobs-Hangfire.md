...
[Hangfire](https://www.hangfire.io/) is an advanced background job manager. You can integrate Hangfire with the JELLOG Framework to use it instead of the [default background job manager](Background-Jobs.md). In this way, you can use the same background job API for Hangfire and your code will be independent of Hangfire. If you like, you can directly use Hangfire's API, too.
...
````bash
jellog add-package DataGap.Jellog.BackgroundJobs.HangFire
````
...
1. Add the [DataGap.Jellog.BackgroundJobs.HangFire](https://www.nuget.org/packages/DataGap.Jellog.BackgroundJobs.HangFire) NuGet package to your project:

   ````
   Install-Package DataGap.Jellog.BackgroundJobs.HangFire
   ````

2. Add the `JellogBackgroundJobsHangfireModule` to the dependency list of your module:

````csharp
[DependsOn(
    //...other dependencies
    typeof(JellogBackgroundJobsHangfireModule) //Add the new module dependency
    )]
...
You can integrate the Hangfire dashboard to [JELLOG authorization system](Authorization.md) using the **JellogHangfireAuthorizationFilter**
class. This class is defined in the `DataGap.Jellog.Hangfire` package. The following example, checks if the current user is logged in to the application:
...
* `JellogHangfireAuthorizationFilter` is an implementation of an authorization filter.

#### JellogHangfireAuthorizationFilter

`JellogHangfireAuthorizationFilter` class has the following fields:
...
app.UseHangfireDashboard("/hangfire", new DashboardOptions
{
    AsyncAuthorization = new[] { new JellogHangfireAuthorizationFilter(requiredPermissionName: "MyHangFireDashboardPermissionName") }
});
```
