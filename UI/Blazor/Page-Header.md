...
You can use the`PageHeader` component to set the page title, the breadcrumb items and the toolbar items for a page. Before using the `PageHeader` component, you need to add a using statement for the `DataGap.Jellog.AspNetCore.Components.Web.Theming.Layout` namespace.
...
Create a collection of `DataGap.Jellog.BlazoriseUI.BreadcrumbItem` objects and set the collection to the `BreadcrumbItems` parameter.
