...
* The [LeptonX Theme](https://x.leptontheme.com/) is a theme that has a [commercial](https://docs.jellog.io/en/commercial/latest/themes/lepton-x/commercial/blazor) and a [lite](../../Themes/LeptonXLite/Blazor.md) version. 
...
All the themes must depend on the [DataGap.Jellog.AspNetCore.Components.WebAssembly.Theming](https://www.nuget.org/packages/DataGap.Jellog.AspNetCore.Components.WebAssembly.Theming) NuGet package, so they are indirectly depending on the following libraries:

{{end}}

{{if UI == "BlazorServer"}}

All the themes must depend on the [DataGap.Jellog.AspNetCore.Components.Server.Theming](https://www.nuget.org/packages/DataGap.Jellog.AspNetCore.Components.Server.Theming) NuGet package, so they are indirectly depending on the following libraries:
...
A theme generally needs to add a global style to the page. JELLOG provides a system to manage the [Global Styles and Scripts](Global-Scripts-Styles.md). A theme can implement the `IBundleContributor` to add global style or script files to the page.
...
`styles.css` file should be added into the `wwwroot` folder of the theme project for this example. When you use the `jellog bundle` command, this class is automatically discovered and executed to add the style to the page.
...
Local Storage is used to get and set the current language with the `Jellog.SelectedLanguage` key.
...
// Redirect to JELLOG language switch endpoint.
NavigationManager.NavigateTo(
    $"/Jellog/Languages/Switch?culture={newLanguage.CultureName}&uiCulture={newLanguage.UiCultureName}&returnUrl={relativeUrl}",
    forceLoad: true
);
````
{{end}}
