...
A Data Table (aka Data Grid) is a UI component to show tabular data to the users. There are a lot of Data table components/libraries and **you can use any one you like** with the JELLOG Framework. However, the startup templates come with the [DataTables.Net](https://datatables.net/) library as **pre-installed and configured**. JELLOG Framework provides adapters for this library and make it easy to use with the API endpoints.
...
First of all, you can follow the official documentation to understand how the [DataTables.Net](https://datatables.net/) works. This section will focus on the JELLOG addons & integration points rather than fully covering the usage of this library.

### A Quick Example

You can follow the [web application development tutorial](https://docs.jellog.io/en/jellog/latest/Tutorials/Part-1?UI=MVC) for a complete example application that uses the DataTables.Net as the Data Table. This section shows a minimalist example.
...
First, add an `jellog-table` as shown below, with an `id`:

````html
<jellog-table striped-rows="true" id="BooksTable"></jellog-table>
````

> `jellog-table` is a [Tag Helper](Tag-Helpers/Index.md) defined by the JELLOG Framework, but a simple `<table...>` tag would also work.
...
The example code above uses some JELLOG integration features those will be explained in the next sections.

### Configuration Normalization

`jellog.libs.datatables.normalizeConfiguration` function takes a DataTables configuration and normalizes to simplify it;
...
`normalizeConfiguration` uses the default configuration. You can change the default configuration using the `jellog.libs.datatables.defaultConfigurations` object. Example:

````js
jellog.libs.datatables.defaultConfigurations.scrollX = false;
````
...
DataTables.Net has its own expected data format while getting results of an AJAX call to the server to get the table data. They are especially related how paging and sorting parameters are sent and received. JELLOG Framework also offers its own conventions for the client-server [AJAX](JavaScript-API/Ajax.md) communication. 

The `jellog.libs.datatables.createAjax` method (used in the example above) adapts request and response data format and perfectly works with the [Dynamic JavaScript Client Proxy](Dynamic-JavaScript-Proxies.md) system.
...
ajax: jellog.libs.datatables.createAjax(acme.bookStore.books.book.getList, inputAction, responseCallback)
````
...
````js
ajax: jellog.libs.datatables.createAjax(
...
`rowAction` is an option defined by the JELLOG Framework to the column definitions to show a drop down button to take actions for a row in the table.
...
````js
visible: jellog.auth.isGranted('BookStore.Books.Delete');
````
...
`jellog.libs.datatables.defaultRenderers` option allows you to define new data formats and set renderers for them.

**Example: Render male / female icons based on the gender**

````js
jellog.libs.datatables.defaultRenderers['gender'] = function(value) {
    if (value === 'f') {
...
You can use any library you like. For example, [see this article](https://community.jellog.io/articles/using-devextreme-components-with-the-jellog-framework-zb8z7yqv) to learn how to use DevExtreme Data Grid in your applications.
