...
using DataGap.Jellog.Identity.Web.Pages.Identity.Users;
...
You typically want to copy the original `.cshtml` file of the module, then make the necessary changes. You can find the original file [here](https://github.com/jellogframework/jellog/blob/dev/modules/account/src/DataGap.Jellog.Account.Web/Pages/Account/Login.cshtml). Do not copy the `Login.cshtml.cs` file which is the code behind file for the razor page and we don't want to override it yet (see the next section).
...
Create a page model class deriving from the ` LoginModel ` (defined in the ` DataGap.Jellog.Account.Web.Pages.Account ` namespace):
...
public class MyLoginModel : LoginModel
{
    public MyLoginModel(
        ...
        IOptions<JellogAccountOptions> accountOptions
        ) : base(
        schemeProvider,
        accountOptions)
    {
...
You don't have to inherit from the original page model class (like done in the previous example). Instead, you can completely **re-implement** the page yourself. In this case, just derive from `PageModel`, `JellogPageModel` or any suitable base class you need.

## Overriding a View Component

The JELLOG Framework, pre-built themes and modules define some **re-usable view components**. These view components can be replaced just like a page described above.
...
If you need, you can also replace [the code behind c# class](https://github.com/jellogframework/jellog/blob/dev/modules/basic-theme/src/DataGap.Jellog.AspNetCore.Mvc.UI.Theme.Basic/Themes/Basic/Components/Brand/MainNavbarBrandViewComponent.cs) of the component just using the dependency injection system.
...
For example, JELLOG Framework defines a **global style bundle** which is added to every page (actually, added to the layout by the themes). Let's add a **custom style file** to the end of the bundle files, so we can override any global style.
...
First, create a standard JavaScript file under the `wwwroot`, `Pages` or `Views` folder (JELLOG support to add static resources inside these folders by default). We prefer the `Pages/Identity/Roles` folder to follow the conventions:
...
````js
$(function() {
    jellog.log.info('My custom role script file has been loaded!');
});
````
...
Configure<JellogBundlingOptions>(options =>
{
    options.ScriptBundles
        .Configure(
            typeof(DataGap.Jellog.Identity.Web.Pages.Identity.Roles.IndexModel).FullName,
            bundleConfig =>
            {
...
`typeof(DataGap.Jellog.Identity.Web.Pages.Identity.Roles.IndexModel).FullName` is the safe way to get the bundle name for the role management page.
...
There are two **standard menus** defined by the JELLOG Framework:
...
````csharp
public class NotificationViewComponent : JellogViewComponent
{
...
Finally, you need to add this contributor to the `JellogToolbarOptions`, in the `ConfigureServices` of your module:
...
JELLOG Framework uses the `ITheme` service to get the layout location by the layout name. You can replace this service to dynamically select the layout location.
