...
While you can continue to use the standard [Bootstrap way](https://getbootstrap.com/docs/4.5/components/modal/) to create, open and manage modals in your applications, JELLOG Framework provides a **flexible** way to manage modals by **automating common tasks** for you.

**Example: A modal dialog to create a new role entity**

![modal-manager-example-modal](../../images/modal-manager-example-modal.png)

JELLOG Framework provides the following benefits for such a modal with a form inside it;
...
<jellog-modal>
    <jellog-modal-header title="Product Information"></jellog-modal-header>
    <jellog-modal-body>
...
* It uses [jellog-modal tag helper](Tag-Helpers/Modals.md) to simplify creating the modal HTML code. You can use the standard  Bootstrap modal code if you prefer it.
...
{
    public class ProductInfoModalModel : JellogPageModel
    {
...
First, create an `jellog.ModalManager` object by setting the `viewUrl`, in the JavaScript file of the page that will use the modal:

````js
var productInfoModal = new jellog.ModalManager({
    viewUrl: '/Products/ProductInfoModal'
});
````

> If you only need to specify the `viewUrl`, you can directly pass it to the `ModalManager` constructor, as a shortcut. Example: `new jellog.ModalManager('/Products/ProductInfoModal');`
...
`jellog.ModalManager` handles various common tasks (described in the introduction) when you want to use a form inside the modal.
...
<form method="post" action="@Url.Page("/Products/ProductCreateModal")">
    <jellog-modal>
        <jellog-modal-header title="Create New Product"></jellog-modal-header>
        <jellog-modal-body>
            <jellog-input asp-for="Product.Name"/>
            <jellog-input asp-for="Product.Description"/>
            <jellog-input asp-for="Product.ReleaseDate"/>
        </jellog-modal-body>
        <jellog-modal-footer buttons="@JellogModalButtons.Save | @JellogModalButtons.Cancel"></jellog-modal-footer>
    </jellog-modal>
</form>
````

* The `jellog-modal` has been wrapped by the `form`. This is needed to place the `Save` and the `Cancel` buttons into the form. In this way, the `Save` button acts as the `submit` button for the `form`.
* Used the [jellog-input tag helpers](Tag-Helpers/Form-Elements.md) to simplify to create the form elements. Otherwise, you need to write more HTML.
...
namespace MyProject.Web.Pages.Products
{
    public class ProductCreateModalModel : JellogPageModel
    {
        [BindProperty]
...
* `OnPostAsync` returns `NoContent` (this method is defined by the base `JellogPageModel` class). Because we don't need to a return value in the client side, after the form post operation.
...
* `jellog-input` Tag Helper can understand the data annotation attributes and uses them to shape and validate the form elements. See the [jellog-input tag helpers](Tag-Helpers/Form-Elements.md) document to learn more.

#### Defining the Modal Manager

Again, create an `jellog.ModalManager` object by setting the `viewUrl`, in the JavaScript file of the page that will use the modal:

````js
var productCreateModal = new jellog.ModalManager({
...
````js
jellog.modals.ProductInfo = function () {
...
* This code simply adds a `ProductInfo` class into the `jellog.modals` namespace. The `ProductInfo` class exposes a single public function: `initModal`.
...
````html
<jellog-script src="/Pages/Products/ProductInfoModal.js"/>
<jellog-script src="/Pages/Products/Index.js"/>
````

* We've use the `jellog-script` Tag Helper here. See the [Bundling & Minification](Bundling-Minification.md) document if you want to understand it. You can use the standard `script` tag. It doesn't matter for this case.
...
var productInfoModal = new jellog.ModalManager({
    viewUrl: '/Products/ProductInfoModal',
    modalClass: 'ProductInfo' //Matches to the jellog.modals.ProductInfo
});
````
...
Alternatively, you can define the `jellog.modals.ProductInfo` class in the page's main JavaScript file if the modal is only and always used in the same page. In this case, you don't need to another external script file at all.
...
* `modalClass` (optional, `string`): A JavaScript class defined in the `jellog.modals` namespace that can be used to execute code related to the modal.
...
````js
var myModal = new jellog.ModalManager({
    //...options
});
