...
JELLOG framework can work with any type of client side package management systems. You can even decide to use no package management system and manage your dependencies manually.

However, JELLOG framework works best with **NPM/Yarn**. By default, built-in modules are configured to work with NPM/Yarn.
...
JELLOG is a modular platform. Every developer can create modules and the modules should work together in a **compatible** and **stable** state.
...
To solve the versioning problem, we created a **standard set of packages** those depends on some common third-party libraries. Some example packages are [@jellog/jquery](https://www.npmjs.com/package/@jellog/jquery), [@jellog/bootstrap](https://www.npmjs.com/package/@jellog/bootstrap) and [@jellog/font-awesome](https://www.npmjs.com/package/@jellog/font-awesome). You can see the **list of packages** from the [GitHub repository](https://github.com/dataGapsoft/jellog/tree/master/npm/packs).
...
If you need a third-party NPM package that is not in the standard set of packages, you can create a Pull Request on the Github [repository](https://github.com/dataGapsoft/jellog). A pull request that follows these rules is accepted:

* Package name should be named as `@jellog/package-name` for a `package-name` on NPM (example: `@jellog/bootstrap` for the `bootstrap` package).
* It should be the **latest stable** version of the package.
* It should only depend a **single** third-party package. It can depend on multiple `@jellog/*` packages.
* The package should include a `jellog.resourcemapping.js` file formatted as defined in the *Mapping The Library Resources* section. This file should only map resources for the depended package.
...
JELLOG CLI's `jellog install-libs` command **copies resources** from **node_modules** to **wwwroot/libs** folder. Each **standard package** (see the *@JELLOG NPM Packages* section) defines the mapping for its own files. So, most of the time, you only configure dependencies.
...
A module should define a JavaScript file named `jellog.resourcemapping.js` which is formatted as in the example below:
...
Once you properly configure the `jellog.resourcemapping.js` file, you can run the following JELLOG CLI command from the command line:

````bash
jellog install-libs
````

When you run this command, all packages will copy their own resources into the **wwwroot/libs** folder. Running `jellog install-libs` is only necessary if you make a change in your dependencies in the **package.json** file.
