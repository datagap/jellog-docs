...
The @jellog/utils package provides a useful data structure known as a [doubly linked list](https://en.wikipedia.org/wiki/Doubly_linked_list). It is availabe in both Angular (via an import) and MVC (via `jellog.utils.common` global object).
...
```js
import { LinkedList } from '@jellog/utils';
...
```js
var list = new jellog.utils.common.LinkedList();
```
