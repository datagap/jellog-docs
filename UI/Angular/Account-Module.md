...
Install the `@jellog/ng.account` NPM package by running the below command:

```bash
npm install @jellog/ng.account
```
...
const routes: Routes = [
  ...
  {
    path: 'account',
    loadChildren: () => import('@jellog/ng.account').then(m => m.AccountModule.forLazy()),
  },
  //...
...
The pro startup template comes with `@dataGap/jellog.ng.account` package. You should update the package version to v4.3 or higher version. The package can be updated by running the following command:

```bash
npm install @dataGap/jellog.ng.account
```
> Make sure v4.3 or higher version is installed.
...
const routes: Routes = [
  ...
  {
    path: 'account',
    loadChildren: () => import('@dataGap/jellog.ng.account/public').then(m => m.AccountPublicModule.forLazy()),
  },
  //...
export class AppRoutingModule {}
```
