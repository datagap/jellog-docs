...
```html
<h1>{%{{{ '::Key' | jellogLocalization }}}%}</h1>

<h1>{%{{{ 'MyProjectName::Key' | jellogLocalization }}}%}</h1>
```

## Using the Localization Pipe

You can use the `jellogLocalization` pipe to get localized text as in this example:

```html
<h1>{%{{{ 'Resource::Key' | jellogLocalization }}}%}</h1>
```
...
```html
<h1>
  {%{{{ { key: 'Resource::Key', defaultValue: 'Default Value' } | jellogLocalization }}}%}
</h1>
```
...
{
  //...
  JellogAccount: { // JellogAccount is the resource name
    Key: "Value",
...
```html
<h1>{%{{{ 'JellogAccount::PagerInfo' | jellogLocalization:'20':'30':'50' }}}%}</h1>
...
First of all you should import the `LocalizationService` from **@jellog/ng.core**

```js
import { LocalizationService } from '@jellog/ng.core';
...
this.localizationService.instant(
  'JellogIdentity::UserDeletionConfirmation',
  'John'
);
...
this.localizationService.instant(
  {
    key: 'JellogIdentity::UserDeletionConfirmation',
...
```html
<div>{%{{{ 'MyProjectName::Administration' | jellogLocalization }}}%}</div>

<div>{%{{{ 'MyProjectName::HomePage' | jellogLocalization }}}%}</div>
```
...
As of v2.9 JELLOG has RTL support. If you are generating a new project with v2.9 and above, everything is set, you do not need to do any changes. If you are migrating your project from an earlier version, please follow the 2 steps below:
...
Since JELLOG has more than one language, Angular locale files loads lazily using [Webpack's import function](https://webpack.js.org/api/module-methods/#import-1) to avoid increasing the bundle size and register to Angular core using the [`registerLocaleData`](https://angular.io/api/common/registerLocaleData) function. The chunks to be included in the bundle are specified by the [Webpack's magic comments](https://webpack.js.org/api/module-methods/#magic-comments) as hard-coded. Therefore a `registerLocale` function that returns Webpack `import` function must be passed to `CoreModule`.

### registerLocaleFn

`registerLocale` function that exported from `@jellog/ng.core/locale` package is a higher order function that accepts `cultureNameLocaleFileMap` object and `errorHandlerFn` function as params and returns Webpack `import` function. A `registerLocale` function must be passed to the `forRoot` of the `CoreModule` as shown below:
...
import { storeLocaleData } from '@jellog/ng.core/locale';
import(
...
import { differentLocales } from '@jellog/ng.core';
