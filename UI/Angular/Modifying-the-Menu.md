...
You can define your routes by adding `routes` as a child property to `data` property of a route configuration in the `app-routing.module`. The `@jellog/ng.core` package organizes your routes and stores them in the `RoutesService`.
...
```js
{
  ...
  data: {
    ...
    ] as JELLOG.Route[], // can be imported from @jellog/ng.core
  },
}
```
...
// eThemeSharedRouteNames enum can be imported from @jellog/ng.theme.shared

const dashboardRouteConfig: JELLOG.Route = {
  path: '/dashboard',
  name: '::Menu:Dashboard',
...
const newHomeRouteConfig: Partial<JELLOG.Route> = {
