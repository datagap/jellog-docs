...
const twoFactorBehaviour = this.config.getSetting("Jellog.Identity.TwoFactor.Behaviour");

// or
this.config.getSetting$("Jellog.Identity.TwoFactor.Behaviour").subscribe(twoFactorBehaviour => {
...
Please refer to `ApplicationConfigurationDto` type for all the properties you can get with `getOne` and `getDeep`. It can be found in the [models.ts file](https://github.com/jellogframework/jellog/blob/dev/npm/ng-packs/packages/core/src/lib/proxy/dataGap/jellog/asp-net-core/mvc/application-configurations/models.ts#L11).
...
```js
import {ApplicationConfigurationService, ConfigStateService} from '@jellog/ng.core';
