...
JELLOG Angular UI supports the multi-tenancy. The following features related to multi-tenancy are available in the startup templates.
...
- `https://mytenant1.identity.mydomain.com` as `JellogIdentity` remote endpoint URL.

The app sends the `__tenant` header that contains the current tenant id on each request.

## See Also

- [Multi Tenancy in JELLOG](../../Multi-Tenancy.md)
