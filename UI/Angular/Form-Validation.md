...
Reactive forms in JELLOG Angular UI are validated by [ngx-validate](https://www.npmjs.com/package/@ngx-validate/core) and helper texts are shown automatically based on validation rules and error blueprints. You do not have to add any elements or components to your templates. The library handles that for you. Here is how the experience is:
...
import { DEFAULT_VALIDATION_BLUEPRINTS } from "@jellog/ng.theme.shared";
...
Alternatively, you may provide the `VALIDATION_BLUEPRINTS` token directly in your root module. Please do not forget to spread `DEFAULT_VALIDATION_BLUEPRINTS`. Otherwise, built-in JELLOG validation messages will not work.
...
```js
import { ValidationErrorComponent } from "@jellog/ng.theme.basic";
...
@Component({
  ...
  template: `
    <div
      ...
      *ngFor="let error of jellogErrors; trackBy: trackByFn"
    >
      {%{{{ error.message | jellogLocalization: error.interpoliteParams }}}%}
    </div>
  `,
