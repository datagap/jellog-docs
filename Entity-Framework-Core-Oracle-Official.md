...
Find ***YourProjectName*EntityFrameworkCoreModule** class inside the `.EntityFrameworkCore` project, remove `typeof(JellogEntityFrameworkCoreSqlServerModule)` from the `DependsOn` attribute, add `typeof(JellogEntityFrameworkCoreOracleModule)`

Also replace `using DataGap.Jellog.EntityFrameworkCore.SqlServer;` with `using DataGap.Jellog.EntityFrameworkCore.Oracle;`.
