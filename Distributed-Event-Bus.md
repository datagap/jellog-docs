...
`EventName` attribute is optional, but suggested. If you don't declare it for an event type (ETO class), the event name will be the full name of the event class, `JellogDemo.StockCountChangedEto` in this case.
...
* `MyHandler` is **automatically discovered** by the JELLOG Framework and `HandleEventAsync` is called whenever a `StockCountChangedEto` event occurs.
* If you are using a distributed message broker, like RabbitMQ, JELLOG automatically **subscribes to the event on the message broker**, gets the message, executes the handler.
...
JELLOG Framework **automatically publishes** distributed events for **create, update and delete** operations for an [entity](Entities.md) once you configure it.
...
using System.Threading.Tasks;
using DataGap.Jellog.DependencyInjection;
using DataGap.Jellog.Domain.Entities.Events.Distributed;
using DataGap.Jellog.EventBus.Distributed;
...
You can configure the `JellogDistributedEntityEventOptions` in the `ConfigureServices` of your [module](Module-Development-Basics.md) to add a selector.

**Example: Configuration samples**

````csharp
Configure<JellogDistributedEntityEventOptions>(options =>
{
...
Once you enable **auto events** for an entity, JELLOG Framework starts to publish events on the changes on this entity. If you don't specify a corresponding **E**vent **T**ransfer **O**bject (ETO) for the entity, JELLOG Framework uses a standard type, named `EntityEto`, which has only two properties:
...
Then you can use the `JellogDistributedEntityEventOptions.EtoMappings` option to map your `Product` entity to the `ProductEto`:
...
That means you can define more than one configuration for outbox (also for inbox) with different names. JELLOG runs all the configured outboxes.
...
JELLOG provides `IEventOutbox` and `IEventInbox` abstractions as extension point for the outbox/inbox system. You can create classes by implementing these interfaces and register them to [dependency injection](Dependency-Injection.md).

Once you implement your custom event boxes, you can configure `JellogDistributedEventBusOptions` to use your event box classes:
