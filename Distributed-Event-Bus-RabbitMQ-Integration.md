...
Use the JELLOG CLI to add [DataGap.Jellog.EventBus.RabbitMQ](https://www.nuget.org/packages/DataGap.Jellog.EventBus.RabbitMQ) NuGet package to your project:

* Install the [JELLOG CLI](https://docs.jellog.io/en/jellog/latest/CLI) if you haven't installed before.
* Open a command line (terminal) in the directory of the `.csproj` file you want to add the `DataGap.Jellog.EventBus.RabbitMQ` package.
* Run `jellog add-package DataGap.Jellog.EventBus.RabbitMQ` command.

If you want to do it manually, install the [DataGap.Jellog.EventBus.RabbitMQ](https://www.nuget.org/packages/DataGap.Jellog.EventBus.RabbitMQ) NuGet package to your project and add `[DependsOn(typeof(JellogEventBusRabbitMqModule))]` to the [JELLOG module](Module-Development-Basics.md) class inside your project.
...
````csharp
Configure<JellogRabbitMqEventBusOptions>(options =>
{
