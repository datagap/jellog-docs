...
JELLOG Feature system is used to **enable**, **disable** or **change the behavior** of the application features **on runtime**.
...
> The feature system is implemented with the [DataGap.Jellog.Features](https://www.nuget.org/packages/DataGap.Jellog.Features) NuGet package. Most of the times you don't need to manually [install it](https://jellog.io/package-detail/DataGap.Jellog.Features) since it comes pre-installed with the [application startup template](Startup-Templates/Application.md).
...
`[RequiresFeature]` attribute (defined in the `DataGap.Jellog.Features` namespace) is used to declaratively check if a feature is `true` (enabled) or not. It is a useful shortcut for the `boolean` features.
...
* `RequiresFeature` may get multiple feature names, like `[RequiresFeature("Feature1", "Feature2")]`. In this case JELLOG checks if any of the features enabled. Use `RequiresAll` option, like `[RequiresFeature("Feature1", "Feature2", RequiresAll = true)]` to force to check all of the features to be enabled.
* Multiple usage of `[RequiresFeature]` attribute is supported for a method or class. JELLOG check checks all of them in that case.

> Feature name can be any arbitrary string. It should be unique for a feature.

#### About the Interception

JELLOG Framework uses the interception system to make the `[RequiresFeature]` attribute working. So, it can work with any class (application services, controllers...) that is injected from the [dependency injection](Dependency-Injection.md).
...
> There is an exception for the **controller and razor page methods**. They **don't require** the following the rules above, since JELLOG Framework uses the action/page filters to implement the feature checking in this case.
...
```csharp
public class ProductController : JellogController
{
    private readonly IFeatureChecker _featureChecker;
...
* `CheckEnabledAsync(string name)`: Checks if given feature is enabled. Throws an `JellogAuthorizationException` if the feature was not `true` (enabled).
...
> JELLOG automatically discovers this class and registers the features. No additional configuration required.
...
using DataGap.Jellog.Validation.StringValues;
...
* `EditionFeatureValueProvider` tries to get the feature value for the current edition. Edition Id is obtained from the current principal identity (`ICurrentPrincipalAccessor`) with the claim name `editionid` (a constant defined as`JellogClaimTypes.EditionId`). Editions are not implemented for the [tenant management](Modules/Tenant-Management.md) module. You can implement it yourself or consider to use the [SaaS module](https://commercial.jellog.io/modules/DataGap.Saas) of the JELLOG Commercial.
...
Once a provider is defined, it should be added to the `JellogFeatureOptions` as shown below:

```csharp
Configure<JellogFeatureOptions>(options =>
{
