...
JELLOG BLOB Storage system is also compatible to other JELLOG Framework features like [multi-tenancy](Multi-Tenancy.md).

## BLOB Storage Providers

The JELLOG Framework has already the following storage provider implementations:
...
More providers will be implemented by the time. You can [request](https://github.com/jellogframework/jellog/issues/new) it for your favorite provider or [create it yourself](Blob-Storing-Custom-Provider.md) and [contribute](Contribution/Index.md) to the JELLOG Framework.
...
[DataGap.Jellog.BlobStoring](https://www.nuget.org/packages/DataGap.Jellog.BlobStoring) is the main package that defines the BLOB storing services. You can use this package to use the BLOB Storing system without depending a specific storage provider.
...
* Open a command line (terminal) in the directory of the `.csproj` file you want to add the `DataGap.Jellog.BlobStoring` package.
* Run `jellog add-package DataGap.Jellog.BlobStoring` command.

If you want to do it manually, install the [DataGap.Jellog.BlobStoring](https://www.nuget.org/packages/DataGap.Jellog.BlobStoring) NuGet package to your project and add `[DependsOn(typeof(JellogBlobStoringModule))]` to the [JELLOG module](Module-Development-Basics.md) class inside your project.
...
> If you don't use the `BlobContainerName` attribute, JELLOG Framework uses the full name of the class (with namespace), but it is always recommended to use a container name which is stable and does not change even if you rename the class.
...
`JellogBlobStoringOptions` is the [options class](Options.md) to configure the containers. You can configure the options inside the `ConfigureServices` method of your [module](Module-Development-Basics.md).
...
* `IBlobContainerConfigurationProvider` is used to get the `BlobContainerConfiguration` for a given container name. Default implementation (`DefaultBlobContainerConfigurationProvider`) gets the configuration from the `JellogBlobStoringOptions` explained above.
