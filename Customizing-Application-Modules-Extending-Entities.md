...
To overcome the difficulties described above, JELLOG Framework entity extension system for the Entity Framework Core that allows you to use the same extra properties API defined above, but store a desired property as a separate field in the database table.
...
This handler is executed only when a new tenant has been created. All the pre-built JELLOG [application modules](Modules/Index.md) define corresponding `ETO` types for their entities. So, you can easily get informed when they changes.

> Notice that JELLOG doesn't publish distributed events for an entity by default. Because it has a cost and should be enabled by intention. See the [distributed event bus document](Distributed-Event-Bus.md) to learn more.
