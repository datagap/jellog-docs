# Integrating MAUI Client via using OpenID Connect
 This is a demonstration for connecting JELLOG backend from MAUI app via using openid connect.
...
## Source Code
You can also find source code on GitHub in JELLOG-Samples.
- [jellogframework/jellog-samples/MAUI-OpenId](https://github.com/jellogframework/jellog-samples/tree/master/MAUI-OpenId)

## Creating projects
- Create an JELLOG project without UI

```bash
jellog new Acme.BookStore -t app --no-ui -d mongodb --no-random-ports
```
...
AccessToken will be retrieved.
    ...
    ![jellog-maui-android-openid-connect](art/android-login-demo.gif)
...
- Make all steps for MacCatalyst, too.
 ...
 > ![jellog-maui-example-compiler-select](art/compiler-select.png)


![jellog-maui-ios-openid-connect](art/ios-login-demo.gif)

![jellog-maui-MacCatalyst-openid-connect](art/macos-login-demo.gif)
...
The purpose of this arcitle is connecting to JELLOG backend with access token and it's working properly.

I'm planning to integrate HttpApi.Client library of backend project instead of making requests manually as a second part of this article. I'll get inspired by [hikalkan/maui-jellog-playing](https://github.com/hikalkan/maui-jellog-playing) repo to achive that.
