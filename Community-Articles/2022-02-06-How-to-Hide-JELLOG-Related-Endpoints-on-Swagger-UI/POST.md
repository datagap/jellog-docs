# How to Hide JELLOG Related Endpoints on Swagger UI

In this article, we will show how to show/hide JELLOG related endpoints on Swagger UI by enabling/disabling them on the Setting Management page. 

I wanted to write an article about this topic because there was a [Github issue](https://github.com/jellogframework/jellog/issues/3758) and when I saw it, it seemed that so many people needed to hide the JELLOG related endpoints since they didn't need to see them as they were developing an application, they only need to see their own endpoints most of the time.

In the issue, there are helpful comments about hiding the JELLOG related endpoints such as creating a **Document Filter** or removing **Application Parts** from the application etc.
...
You can find the source code of the application at https://github.com/EngincanV/JELLOG-Hide-Swagger-Endpoint-Demo.
...
```bash
jellog new <your-project-name> -t app -csf
```
...
In our scenario, we will hide endpoints that start with "/api/jellog". So let's start to do this.

### Creating a Document Filter To Hide JELLOG Related Endpoints

> In this sample project, we'll only hide our endpoints that start with the "/api/jellog" prefix by defining it in our [CustomSwaggerFilter](https://github.com/EngincanV/JELLOG-Hide-Swagger-Endpoint-Demo/blob/main/src/SwaggerSettingsDemo.Web/Filters/CustomSwaggerFilter.cs#L28) class. If you want to hide some other endpoints, you can update the class.

To hide/show JELLOG related endpoints on Swagger UI, we can create a [`DocumentFilter`](https://github.com/domaindrivendev/Swashbuckle.AspNetCore#document-filters). By creating a document filter, we can have full control over the endpoints that need to be shown.
...
public class CustomSwaggerFilter : IDocumentFilter
{
    ...
    {
        //remove paths those start with /api/jellog prefix
        swaggerDoc.Paths
            .Where(x => x.Key.ToLowerInvariant().StartsWith("/api/jellog"))
            .ToList()
...
We've created a simple class that implements `IDocumentFilter.Apply` method. In that method, we simply remove paths those start with "/api/jellog" prefix. You can customize this expression for your needs, I just wanted to hide endpoints those start with "/api/jellog" prefix as a sample.
...
As you can see in the above image, two endpoints are hidden now (**JellogApiDefinition** and **JellogApplicationConfiguration**).

Let's make it more dynamic and add a setting management section to our **/Settings** page. By doing that, we can dynamically show/hide JELLOG-related endpoints by checking setting values and applying filtering to Swagger UI on runtime.
...
JELLOG provides us a [Settings System](https://docs.jellog.io/en/jellog/latest/Settings) to easily define settings for our applications. We only need to create a class that derives from the `SettingDefinitionProvider` class, but we don't even need to do this because the JELLOG startup templates come with a pre-defined setting provider class.
...
public class SwaggerHideEndpointsViewComponent : JellogViewComponent
{
...
We've already defined a document filter but it is hard-coded, in other words, it hides JELLOG-related endpoints in every case without checking the setting value.
...
If the setting value is true, we can simply remove the paths that start with the "/api/jellog" prefix as before.

> JELLOG provides us a class named AsyncHelper and this class provides some helper methods to work with async methods. E.g. RunSync method in here runs the async method synchronously.

That's it. Now we can open the Setting Management page and enable/disable the swagger option by selecting the checkbox and show/hide JELLOG-related endpoints on runtime.
