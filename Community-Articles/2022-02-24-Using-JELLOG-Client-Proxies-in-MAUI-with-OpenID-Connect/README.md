# Using JELLOG Client Proxies in MAUI with OpenID Connect
The purpose of this article is to integrate JELLOG Core into the MAUI project and initialize it as an **JellogModule** then make able consuming API using JELLOG IAppServices.

Before we start, I offer my special thanks to [@hikalkan](https://github.com/hikalkan/maui-jellog-playing) because this repository ( [hikalkan/maui-jellog-playing](https://github.com/hikalkan/maui-jellog-playing) ) is a fantastic inspiration for the purpose of this article.

## Getting Started

In this article, we'll work on an application that was built on the previous article: [Integrating MAUI Client via Using OpenID Connect](https://community.jellog.io/posts/integrating-maui-client-via-using-openid-connect-aqjjwsdf).


## Source Code
Source code is available on GitHub: 
[jellogframework/jellog-samples/MAUI-OpenId](https://github.com/jellogframework/jellog-samples/tree/master/MAUI-OpenId)


## Configuring JELLOG Core

As a first step, Dependency Injection will be changed with module initialization. We have to initialize our application as an JELLOG Module first.

- Add the following dependencies to MAUI Client.

    ```xml
    <PackageReference Include="DataGap.Jellog.Http.Client.IdentityModel" Version="5.1.3" />
    <PackageReference Include="DataGap.Jellog.Autofac" Version="5.1.3" />
    ```
...
- Add HttpApi.Client project reference
    ...
    ```
    And run `jellog build` command under MAUI application folder.

    > `jellog build` command is equivalent of `dotnet build /graphBuild`, it's like a shortcut to graphBuild. The graphBuild finds all dependency tree and build them recursively.
...
- Create **BookStoreMauiClientModule**.
    ...
    public class BookStoreMauiClientModule : JellogModule
    {
        public override void ConfigureServices(ServiceConfigurationContext context)
        {
            ...
            context.Services.AddTransient<HttpClient>(sp =>
                ...
                {
                    // Temporarily. We'll use JELLOG's Proxy for sendind requests.
...
- Add `appsettings.json` file to root path of your application and mark it as **Embedded resource**.
 ...
 - Finally, Go back `MauiApplication.cs` and clear old codes and initialize JELLOG.
    ...
    public static class MauiProgram
    {
        public static MauiApp CreateMauiApp()
        {
            ...
            builder.ConfigureContainer(new JellogAutofacServiceProviderFactory(new Autofac.ContainerBuilder()), containerBuilder =>
            {
            ...
            app.Services.GetRequiredService<IJellogApplicationWithExternalServiceProvider>()
    ...
    Now application is runnable and all behaviors are the same with previos state. But it uses power of JELLOG right now.
...
- Add **AccessTokenRemoteServiceHttpClientAuthenticator.cs** instead.
    ...
    using DataGap.Jellog.Http.Client.Authentication;
    using DependencyAttribute = DataGap.Jellog.DependencyInjection.DependencyAttribute;
        ...
        public async Task SeedAsync(DataSeedContext context)
        {
            ...
            if(count <= 1) // Not sure 'admin' user was seeded before or not.
            {
                ...
                await repository.InsertManyAsync(new []{
                    new IdentityUser(guidGenerator.Create(), "john.doe", "john.doe@jellog.io"),
                    new IdentityUser(guidGenerator.Create(), "Zane.Frost", "Zane.Frost@jellog.io"),
                    new IdentityUser(guidGenerator.Create(), "Oscar.Landry", "Oscar.Landry@jellog.io"),
                    new IdentityUser(guidGenerator.Create(), "Yasemin.Roberts", "Yasemin.Roberts@jellog.io"),
                    new IdentityUser(guidGenerator.Create(), "Yasmine.Perez", "Yasmine.Perez@jellog.io"),
                    new IdentityUser(guidGenerator.Create(), "Tobi.Becker", "Tobi.Becker@jellog.io"),
                    new IdentityUser(guidGenerator.Create(), "Fox.Gilmore", "Fox.Gilmore@jellog.io"),
                    new IdentityUser(guidGenerator.Create(), "Benny.Burris", "Benny.Burris@jellog.io"),
                    new IdentityUser(guidGenerator.Create(), "Chad.Camacho", "Chad.Camacho@jellog.io"),
                });
            }
        }
    }
...
- Create a folder named **Pages** and add a content page named `UsersPage`.
    ...
    ![net-maui-jellog-contentpage-template](art/net-maui-contentpage-template.png)
...
- And use that ViewModel in XAML design page.
    ...
    >
    > ![jellog-maui-demo-xaml-intellisense](art/xaml-intellisense.png)
...
- Add `Shell Pagae (MAUI)` to root of your application with name **AppShell.xaml**.
    ...
    _Additionally, you might want to put [jellog_icon.svg](maui/Acme.BookStore.MauiClient/Resources/Images/jellog_logo.svg) file under your **Resources/Images** folder._
    ```xml
    <?xml version="1.0" encoding="utf-8" ?>
...
| Android| iOS|
| --- | --- |
| ![jellog-maui-android-appservice](art/android-users-demo.gif) | ![ios-jellog-maui](art/ios-users-demo.gif) |

| UWP |
| --- | 
| ![jellog-maui-uwp-appservice](art/uwp-users-demo.gif) |

| MacCatalyst |
| --- |
| ![jellog-maui-MacCatalyst-appservice](art/macos-users-demo.gif) |


## Conclusion
JELLOG Framework can be implemented any platform that runs on dotnet without suffer. JELLOG provides reusable abstractions layers and HttpApi Clients. In this article we've used powerful JELLOG core features such as Dependency Injection, Client Proxies, Validation and more. 
