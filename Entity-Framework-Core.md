...
This document explains how to integrate EF Core as an ORM provider to JELLOG based applications and how to configure it.

## Installation

`DataGap.Jellog.EntityFrameworkCore` is the main NuGet package for the EF Core integration. Install it to your project (for a layered application, to your data/infrastructure layer):

```` shell
Install-Package DataGap.Jellog.EntityFrameworkCore
````

Then add `JellogEntityFrameworkCoreModule` module dependency (`DependsOn` attribute) to your [module](Module-Development-Basics.md):
...
{
    [DependsOn(typeof(JellogEntityFrameworkCoreModule))]
...
Your `DbContext` class should be derived from `JellogDbContext<T>` as shown below:
...
namespace MyCompany.MyProject
{
    public class MyDbContext : JellogDbContext<MyDbContext>
    {
...
You can still use the **data annotation attributes** (like `[Required]`) on the properties of your entity while the JELLOG documentation generally follows the **fluent mapping API** approach. It is up to you.

JELLOG Framework has some **base entity classes** and **conventions** (see the [entities document](Entities.md)) and it provides some useful **extension methods** to configure the properties inherited from the base entity classes.
...
### JellogDbContextOptions

`JellogDbContextOptions` is used to configure the `DbContext` options. When you create a new solution with the JELLOG's application startup template, you will see a simple configuration (in the `EntityFrameworkCore` integration project's module class) as shown below:
...
Use `AddJellogDbContext` method in your module to register your `DbContext` class for [dependency injection](Dependency-Injection.md) system.
...
namespace MyCompany.MyProject
{
    ...
    public class MyModule : JellogModule
    {
        ...
        {
            context.Services.AddJellogDbContext<MyDbContext>();
...
JELLOG can automatically create default [generic repositories](Repositories.md) for the entities in your DbContext. Just use `AddDefaultRepositories()` option on the registration:

````C#
services.AddJellogDbContext<MyDbContext>(options =>
{
...
If you want to replace default repository implementation with your custom repository, do it inside the `AddJellogDbContext` options:

````csharp
context.Services.AddJellogDbContext<BookStoreDbContext>(options =>
{
    options.AddDefaultRepositories();
...
> Actually these are not directly related to the JELLOG Framework. You can follow the [EF Core documentation](https://docs.microsoft.com/en-us/ef/core/querying/related-data/) to learn all the details. This section will cover some topics related to the JELLOG Framework.
...
````csharp
Configure<JellogEntityOptions>(options =>
{
...
* Reference to the `DataGap.Jellog.EntityFrameworkCore` package from your project. In this way, you can directly use `Include` and `ThenInclude` in your code.
...
> Important: You must reference to the `DataGap.Jellog.EntityFrameworkCore` package from the project you want to access to the `DbContext`. This breaks encapsulation, but this is what you want in that case.
...
If your solution is [multi-tenant](Multi-Tenancy.md), tenants may have **separate databases**, you have **multiple** `DbContext` classes in your solution and some of your `DbContext` classes should be usable **only from the host side**, it is suggested to add `[IgnoreMultiTenancy]` attribute on your `DbContext` class. In this case, JELLOG guarantees that the related `DbContext` always uses the host [connection string](Connection-Strings.md), even if you are in a tenant context.
...
> When you use repositories, JELLOG already uses the host database for the entities don't implement the `IMultiTenant` interface. So, most of time you don't need to `[IgnoreMultiTenancy]` attribute if you are using the repositories to work with the database.
...
public class OtherDbContext : JellogDbContext<OtherDbContext>, IBookStoreDbContext
{
    //...
}
```
...
````csharp
context.Services.AddJellogDbContext<OtherDbContext>(options =>
{
    //...
...
JELLOG enables [split queries](https://docs.microsoft.com/en-us/ef/core/querying/single-split-queries) globally by default for better performance. You can change it as needed.
