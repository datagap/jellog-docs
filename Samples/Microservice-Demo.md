...
> This solution is no longer maintained. See [the eShopOnJellog project](https://github.com/jellogframework/eShopOnJellog) for the replacement solution.
...
One of the major goals of the JELLOG framework is to provide a [convenient infrastructure to create microservice solutions](../Microservice-Architecture.md).
...
You can get the source code from [the GitHub repository](https://github.com/jellogframework/jellog-samples/tree/master/MicroserviceDemo).
...
- **IdentityService.Host**: Hosts the JELLOG Identity module which is used to manage users & roles. It has no additional service, but only hosts the Identity module's API.
- **TenantManagementService.Host**: Hosts the JELLOG Tenant Management module which is used to manage roles. It has no additional service, but only hosts the Tenant Management module's API.
- **BloggingService.Host**: Hosts the JELLOG Blogging module which is used to manage blog & posts (a typical blog application). It has no additional service, but only hosts the Blogging module's API.
...
This project is used by all other services and applications for authentication & single sign on. Mainly, uses **IdentityServer4** to provide these services. It uses some of the [pre-build JELLOG modules](../Modules/Index) like *Identity*, *Audit Logging* and *Permission Management*.
...
````csharp
public class AuthServerDbContext : JellogDbContext<AuthServerDbContext>
{
...
These pages are not included in the project itself. Instead, AuthServer project uses the prebuilt JELLOG [account module](https://github.com/jellogframework/jellog/tree/master/modules/account) with IdentityServer extension. That means it can also act as an OpenId Connect server with necessary UI and logic.
...
.AddOpenIdConnect("oidc", options =>
{
    ...
    options.ClaimActions.MapJellogClaimTypes();
});
````
...
The BackendAdminApp.Host project itself has not a single UI element/page. It is only used to serve UI pages of the Identity and Product Management modules. `BackendAdminAppHostModule` adds dependencies to `JellogIdentityWebModule` (*[DataGap.Jellog.Identity.Web](https://www.nuget.org/packages/DataGap.Jellog.Identity.Web)* package) and `ProductManagementWebModule` (*ProductManagement.Web* project) for that purpose.
...
JELLOG application modules generally provides C# client libraries to consume services (APIs) easily (they generally uses the [Dynamic C# API Clients](../API/Dynamic-CSharp-API-Clients.md) feature of the JELLOG framework). That means if you need to consume Identity service API, you can reference to its client package and easily use the APIs by provided interfaces.

For that purpose, `BackendAdminAppHostModule` class declares dependencies for `JellogIdentityHttpApiClientModule` and `ProductManagementHttpApiClientModule`.
...
Since microservices requires authentication & authorization, each remote service call should contain an Authentication header. This header is obtained from the `access_token` inside the current `HttpContext` for the current user. This is automatically done when you use the `DataGap.Jellog.Http.Client.IdentityModel` package. `BackendAdminAppHostModule` declares dependencies to this package and to the related `JellogHttpClientIdentityModelModule` class. It is integrated to the HTTP Clients explained above.
...
The PublicWebSite.Host project has a page to list products (`Pages/Products.cshtml`). It also uses the UI from the blogging module. `PublicWebSiteHostModule` adds dependencies to `BloggingWebModule` (*[DataGap.Blogging.Web](https://www.nuget.org/packages/DataGap.Blogging.Web)* package) for that purpose.
...
Just like explained in the Backend Admin Application section, Public Web Site project also uses the `JellogHttpClientIdentityModelModule` to pass `access_token` to the calling services for authentication.
...
`ConsoleClientDemoModule` has dependencies to `JellogIdentityHttpApiClientModule` and `ProductManagementHttpApiClientModule` in order to use Identity and Product APIs. It also has `JellogHttpClientIdentityModelModule` dependency to authenticate via IdentityServer.
...
.AddIdentityServerAuthentication(options =>
{
    ...
    options.RequireHttpsMetadata = false;    
    options.InboundJwtClaimTypeMap["sub"] = JellogClaimTypes.UserId;
    options.InboundJwtClaimTypeMap["role"] = JellogClaimTypes.Role;
    options.InboundJwtClaimTypeMap["email"] = JellogClaimTypes.Email;
    options.InboundJwtClaimTypeMap["email_verified"] = JellogClaimTypes.EmailVerified;
    options.InboundJwtClaimTypeMap["phone_number"] = JellogClaimTypes.PhoneNumber;
    options.InboundJwtClaimTypeMap["phone_number_verified"] = 
        JellogClaimTypes.PhoneNumberVerified;
    options.InboundJwtClaimTypeMap["name"] = JellogClaimTypes.UserName;
});
````
...
#### JELLOG Configuration Endpoints

JELLOG provides some built-in APIs to get some configuration and information from the server. Examples:

* `/api/jellog/application-configuration` returns localization texts, permission and setting values (try http://localhost:65115/api/jellog/application-configuration for this gateway).
* `/Jellog/ServiceProxyScript` returns dynamic javascript proxies to call services from a javascript client (try http://localhost:65115/Jellog/ServiceProxyScript for this gateway).
...
````csharp
app.MapWhen(ctx => ctx.Request.Path.ToString().StartsWith("/api/jellog/") || 
                   ctx.Request.Path.ToString().StartsWith("/Jellog/"),
    app2 =>
    {
...
This configuration uses standard MVC middleware when request path starts with `/api/jellog/` or `/Jellog/`.

#### Swagger

This gateway is configured to use the [swagger UI](https://swagger.io/tools/swagger-ui/), a popular tool to discover & test HTTP APIs. Normally, Ocelot does not support to show APIs on the swagger, because it can not know details of each microservice API. But it is possible when you follow JELLOG layered module architecture [best practices](../Best-Practices/Index.md).

`BackendAdminAppGatewayHostModule` adds dependency to `JellogIdentityHttpApiModule` (*[DataGap.Jellog.Identity.HttpApi](https://www.nuget.org/packages/DataGap.Jellog.Identity.HttpApi)* package) and `ProductManagementHttpApiModule` (*ProductManagement.HttpApi* project) to include their HTTP API Controllers. In this way, swagger can discover them. While it references to the API layer, it does not reference to the implementation of application services, because they will be running in the related microservice endpoints and redirected by the Ocelot based on the request URL.
...
`ApiName` is the API which is being protected, `PublicWebSiteGateway` in this case. Rest of the configuration is related to claims mapping (which is planned to be automated in next JELLOG versions). The configuration related to authentication in the `appsettings.json` is simple:
...
See the "JELLOG Configuration Endpoints" and "Swagger" topics inside the "Backend Admin Application Gateway" section which are very similar for this gateway.
...
.AddIdentityServerAuthentication(options =>
{
    ...
    options.InboundJwtClaimTypeMap["email_verified"] = JellogClaimTypes.EmailVerified;
    options.InboundJwtClaimTypeMap["phone_number"] = JellogClaimTypes.PhoneNumber;
    options.InboundJwtClaimTypeMap["phone_number_verified"] = JellogClaimTypes.PhoneNumberVerified;
    options.InboundJwtClaimTypeMap["name"] = JellogClaimTypes.UserName;
});
```
...
`ApiName` is the API which is being protected, `InternalGateway` in this case. Rest of the configuration is related to claims mapping (which is planned to be automated in next JELLOG versions). The configuration related to authentication in the `appsettings.json` is simple:
...
This service actually just hosts the JELLOG Identity package/module. Does not include any API itself. In order to host it, adds the following dependencies:

* `JellogIdentityHttpApiModule` (*[DataGap.Jellog.Identity.HttpApi](https://www.nuget.org/packages/DataGap.Jellog.Identity.HttpApi)* package) to provide Identity APIs.
* `JellogIdentityApplicationModule` (*[DataGap.Jellog.Identity.Application](https://www.nuget.org/packages/DataGap.Jellog.Identity.Application)* package) to host the implementation of the application and domain layers of the module.
* `JellogIdentityEntityFrameworkCoreModule` (*[DataGap.Jellog.Identity.EntityFrameworkCore](https://www.nuget.org/packages/DataGap.Jellog.Identity.EntityFrameworkCore)* package) to use EF Core as database API.
...
`ApiName` is the API which is being protected, `IdentityService` in this case. Rest of the configuration is related to claims mapping (which is planned to be automated in next JELLOG versions). The configuration related to authentication in the `appsettings.json` is simple:
...
This service actually just hosts the JELLOG Blogging package/module. Does not include any API itself. In order to host it, adds the following dependencies:

- `BloggingHttpApiModule` (*[DataGap.Blogging.HttpApi](https://www.nuget.org/packages/DataGap.Blogging.HttpApi)* package) to provide Blogging APIs.
- `BloggingApplicationModule` (*[DataGap.Blogging.Application](https://www.nuget.org/packages/DataGap.Blogging.Application)* package) to host the implementation of the application and domain layers of the module.
- `BloggingMongoDbModule` (*[DataGap.Blogging.MongoDB](https://www.nuget.org/packages/DataGap.Jellog.Identity.EntityFrameworkCore)* package) to use MongoDB as the database.
...
`ApiName` is the API which is being protected, `BloggingService` in this case. Rest of the configuration is related to claims mapping (which is planned to be automated in next JELLOG versions). The configuration related to authentication in the `appsettings.json` is simple:
...
When you set `UseCurrentAccessToken` to `false`, JELLOG ignores the current `access_token` in the current `HttpContext` and authenticates to the AuthServer with the credentials defined above.
...
If you check the `JellogPermissionGrants` table in the `MsDemo_Identity` database, you can see the related permission for the `blogging-service-client`.
...
````csharp
public class ProductServiceMigrationDbContext : JellogDbContext<ProductServiceMigrationDbContext>
{
...
`ApiName` is the API which is being protected, `ProductService` in this case. Rest of the configuration is related to claims mapping (which is planned to be automated in next JELLOG versions). The configuration related to authentication in the `appsettings.json` is simple:
...
JELLOG provides a strong infrastructure to make modular application development easier by providing services and architecture (see the [module development best practices guide](../Best-Practices/Index.md)).
...
* `ProductManagement.HttpApi.Client` contains C# proxies to directly use the HTTP API remotely. Uses [Dynamic C# API Clients](../API/Dynamic-CSharp-API-Clients.md) feature of the JELLOG framework.
...
Events are distributed by RabbitMQ for this solution. But JELLOG is message broker independent by providing necessary abstractions (see the [Event Bus](../Event-Bus.md) document).
...
Distributed Events (Event Bus) is a way of messaging where a service raise/trigger events while other services registers/listens to these events to be notified when an important event occurs. JELLOG makes distributed events easier to use by providing conventions, services and integrations.
...
All the integration and communication are done by the JELLOG framework when you use the [DataGap.Jellog.EventBus.RabbitMQ](https://www.nuget.org/packages/DataGap.Jellog.EventBus.RabbitMQ) package. If you need to publish events out of an entity, just inject the `IDistributedEventBus` and use the `PublishAsync` method.
...
[DataGap.Jellog.EventBus.RabbitMQ](https://www.nuget.org/packages/DataGap.Jellog.EventBus.RabbitMQ) package is required to integrate to the RabbitMQ for distributed event system. Then you need to add dependency to the `JellogEventBusRabbitMqModule` for your module. For example, `ProductServiceHostModule` declares this dependency.

`JellogEventBusRabbitMqModule` gets configuration from the `appsettings.json` by default. For example, the Product Service has such a configuration:
...
JELLOG provides automatic audit logging which saves every request in detail (who is the current user, what is the browser/client, what actions performed, which entities changed, even which properties of entities has been updated). See the [audit logging document](../Audit-Logging.md) for details.
...
An Audit Log record has a `CorrelationId` property that can be used to track a request. When a service calls another service in a single web request, they both save audit logs with the same `CorrelationId`. See the `JellogAuditLogs` table in the database.
