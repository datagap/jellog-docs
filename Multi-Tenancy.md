...
Multi-Tenancy is a widely used architecture to create **SaaS applications** where the hardware and software **resources are shared by the customers** (tenants). JELLOG Framework provides all the base functionalities to create **multi tenant applications**. 
...
### JellogMultiTenancyOptions: Enable/Disable Multi-Tenancy

`JellogMultiTenancyOptions` is the main options class to **enable/disable the multi-tenancy** for your application.
...
> Multi-Tenancy is disabled in the JELLOG Framework by default. However, it is **enabled by default** when you create a new solution using the [startup template](Startup-Templates/Application.md). `MultiTenancyConsts` class in the solution has a constant to control it in a single place.

### JellogMultiTenancyOptions: Handle inactive and non-existent tenants.

The `MultiTenancyMiddlewareErrorPageBuilder` of `JellogMultiTenancyOptions` is used to handle inactive and non-existent tenants.
...
JELLOG Framework supports all the following approaches to store the tenant data in the database;
...
`IMultiTenant` interface just defines a `TenantId` property. When you implement this interface, JELLOG Framework **automatically** [filters](Data-Filtering.md) entities for the current tenant when you query from database. So, you don't need to manually add `TenantId` condition while performing queries. A tenant can not access to data of another tenant by default.
...
`ApplicationService`, `DomainService`, `JellogController` and some other base classes already has pre-injected `CurrentTenant` properties. For other type of classes, you can inject the `ICurrentTenant` into your service.
...
JELLOG Framework automatically filters the resources (database, cache...) based on the `ICurrentTenant.Id`. However, in some cases you may want to perform an operation on behalf of a specific tenant, generally when you are in the host context.
...
As mentioned before, JELLOG Framework handles data isolation between tenants using the [Data Filtering](Data-Filtering.md) system. In some cases, you may want to disable it and perform a query on all the data, without filtering for the current tenant.
...
JELLOG Framework provides an extensible **Tenant Resolving** system for that purpose. Tenant Resolving system then used in the **Multi-Tenancy Middleware** to determine the current tenant for the current HTTP request.
...
###### JellogAspNetCoreMultiTenancyOptions

`__tenant` parameter name can be changed using `JellogAspNetCoreMultiTenancyOptions`.

**Example:**

````csharp
services.Configure<JellogAspNetCoreMultiTenancyOptions>(options =>
{
...
import { TENANT_KEY } from '@jellog/ng.core';
...
In a real application, most of times you will want to determine current tenant either by subdomain (like mytenant1.mydomain.com) or by the whole domain (like mytenant.com). If so, you can configure the `JellogTenantResolveOptions` to add the domain tenant resolver.

**Example: Add a subdomain resolver**

````csharp
Configure<JellogTenantResolveOptions>(options =>
{
...
> There is an [example](https://github.com/jellogframework/jellog-samples/tree/master/DomainTenantResolver) that uses the subdomain to determining the current tenant. 

##### Custom Tenant Resolvers

You can add implement your custom tenant resolver and configure the `JellogTenantResolveOptions` in your module's `ConfigureServices` method as like below:
...
> It is recommended to **use the Tenant Management module**, which is already pre-configured when you create a new application with the JELLOG startup templates.

### Other Multi-Tenancy Infrastructure

JELLOG Framework was designed to respect to the multi-tenancy in every aspect and most of the times everything will work as expected.
...
JELLOG Framework provides all the the infrastructure to create a multi-tenant application, but doesn't make any assumption about how you manage (create, delete...) your tenants.
