...
We are excited to announce that the **alpha version** of the LeptonX theme has been released! As stated in [this blog post](https://dataGapsoft.com/blog/introducing-the-lepton-theme-next-generation), LeptonX comes in different shapes. For this release, we introduce only JELLOG based projects with the Angular UI. So, if you are already using the JELLOG Framework and Angular as the frontend choice, you can integrate these packages into your project today.
...
This section shows how to replace the basic theme (that comes with open source JELLOG Framework startup template) with the new LeptonX-lite theme.

To add `LeptonX-lite` into your project,

* Install `@jellog/ng.theme.lepton-x` NPM package

`npm install @jellog/ng.theme.lepton-x@preview` or 
...
```json
"node_modules/@dataGap/ngx-lepton-x.lite/styles/sidemenu-layout.min.css",
...
This section shows how to replace the lepton theme (that comes with the JELLOG Commercial startup template) with the new LeptonX theme.

To add `LeptonX` into your existing projects, 

* Firstly, install `@dataGapsoft/jellog.ng.theme.lepton-x` NPM package

`npm install @dataGapsoft/jellog.ng.theme.lepton-x@preview` or 

`yarn add @dataGapsoft/jellog.ng.theme.lepton-x@preview`
...
```JSON
{
  "input": "node_modules/@dataGapsoft/ngx-lepton-x/styles/themes/dark.css",
  "inject": false,
  "bundleName": "lepton-x.dark"
},
{
  "input": "node_modules/@dataGapsoft/ngx-lepton-x/styles/themes/dim.css",
  "inject": false,
  "bundleName": "lepton-x.dim"
},
{
  "input": "node_modules/@dataGapsoft/ngx-lepton-x/styles/themes/light.css",
  "inject": false,
  "bundleName": "lepton-x.light"
},
"node_modules/@dataGapsoft/ngx-lepton-x/styles/css/sidemenu-layout.min.css",
"node_modules/bootstrap-icons/font/bootstrap-icons.css",
```
...
```js
import { ThemeLeptonXModule } from '@dataGapsoft/jellog.ng.theme.lepton-x';
import { JellogSideMenuLayoutModule } from '@dataGapsoft/jellog.ng.theme.lepton-x/layouts';
...
@NgModule({
  ...
  imports: [
    // ...
    ThemeLeptonXModule.forRoot(),
    JellogSideMenuLayoutModule.forRoot(), // depends on which layout you choose
    // ...
  ],
  // ...
})
...
```js
import { AccountLayoutModule } from '@dataGapsoft/jellog.ng.theme.lepton-x/account';
...
In this blog post, I've explained how to use the alpha version of the new LeptonX theme for JELLOG-based solutions. Please, keep in mind that this is an alpha version, and we will continue to work on the LeptonX theme. The APIs are bound to change and breaking changes may be introduced in future versions. 

We would like you to try it out with the latest version of the JELLOG Framework and give us feedback at lepton@dataGapsoft.com or open an issue on this repository: https://github.com/dataGapsoft/lepton-theme
