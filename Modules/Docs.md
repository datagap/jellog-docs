...
Docs module is an application module for JELLOG framework. It simplifies software documentation. This module is free and open-source.
...
[The documents](https://docs.jellog.io/) for JELLOG framework is also using this module.
...
If you do not have an existing JELLOG project, this step shows you how to create a new project from [jellog.io](https://jellog.io) to add the Docs Module. If you already have an JELLOG project, you can skip this step.

It is recommended to use JELLOG CLI to create new projects. Use the following command:

`jellog new Acme.MyProject`

You can also navigate to https://jellog.io/get-started. Enter your project name as `Acme.MyProject`, other use default options.

Note that this document covers `Entity Framework Core` provider but you can also select `MongoDB` as your database provider.

![Create new project](../images/docs-module_download-new-jellog-project.png)
...
Now an empty JELLOG project has been created! You can now run your project and see the empty website.
...
`jellog add-module DataGap.Docs`

#### 3.2- Manually install

Or you can also manually install nuget package to each project:

* Install [DataGap.Docs.Domain](https://www.nuget.org/packages/DataGap.Docs.Domain/) nuget package to `Acme.MyProject.Domain` project.

  `Install-Package DataGap.Docs.Domain`

* Install [DataGap.Docs.EntityFrameworkCore](https://www.nuget.org/packages/DataGap.Docs.EntityFrameworkCore/) nuget package to `Acme.MyProject.EntityFrameworkCore` project.

  `Install-Package DataGap.Docs.EntityFrameworkCore`

* Install [DataGap.Docs.Application](https://www.nuget.org/packages/DataGap.Docs.Application/) nuget package to `Acme.MyProject.Application` project.

  `Install-Package DataGap.Docs.Application`

* Install [DataGap.Docs.Web](https://www.nuget.org/packages/DataGap.Docs.Domain/) nuget package to `Acme.MyProject.Web` project.

  `Install-Package DataGap.Docs.Web`

##### 3.2.1- Adding Module Dependencies

An JELLOG module must declare `[DependsOn]` attribute if it has a dependency upon another module. Each module has to be added in`[DependsOn]` attribute to the relevant project.
...
* Open `MyProjectDomainModule.cs`and add `typeof(DocsDomainModule)` as shown below;
  ...
  ```csharp
   [DependsOn(
          ...
          typeof(JellogAuditingModule),
          typeof(BackgroundJobsDomainModule),
          typeof(JellogAuditLoggingDomainModule)
          )]
      public class MyProjectDomainModule : JellogModule
      {
          //...
      }
  ```
...
* Open `MyProjectEntityFrameworkCoreModule.cs`and add `typeof(DocsEntityFrameworkCoreModule)` as shown below;
  ...
  ```csharp
      ...
      public class MyProjectEntityFrameworkCoreModule : JellogModule
      {
          //...
      }
  ```
...
* Open `MyProjectApplicationModule.cs`and add `typeof(DocsApplicationModule)` as shown below;
  ...
  ```csharp
     [DependsOn(
      ...
      public class MyProjectApplicationModule : JellogModule
      {
...
Open `package.json` and add `@jellog/docs": "^5.0.0` as shown below:
  ...
  ```json
    {
        ...
        "private": true,
        "dependencies": {
            "@jellog/aspnetcore.mvc.ui.theme.basic": "^5.0.0",
            "@jellog/docs": "^5.0.0"
        }
    }
  ```
...
- Open `MyProjectMigrationsDbContext.cs` and add `builder.ConfigureDocs()` to the `OnModelCreating()`.
  ...
  ```csharp
  public class MyProjectMigrationsDbContext : JellogDbContext<MyProjectMigrationsDbContext>
    {
...
You can use [JELLOG Framework](https://github.com/jellogframework/jellog/) GitHub documents to configure your GitHub document store.

- Name: `JELLOG framework (GitHub)`
...
- ExtraProperties: 
  ...
  ```json
  {"GitHubRootUrl":"https://github.com/jellogframework/jellog/tree/{version}/docs","GitHubAccessToken":"***","GitHubUserAgent":""}
  ```
...
```mssql
INSERT [dbo].[DocsProjects] ([Id], [Name], [ShortName], [Format], [DefaultDocumentName], [NavigationDocumentName], [MinimumVersion], [DocumentStoreType], [ExtraProperties], [MainWebsiteUrl], [LatestVersionBranchName], [ParametersDocumentName]) VALUES (N'12f21123-e08e-4f15-bedb-ae0b2d939658', N'JELLOG framework (GitHub)', N'jellog', N'md', N'Index', N'docs-nav.json', NULL, N'GitHub', N'{"GitHubRootUrl":"https://github.com/jellogframework/jellog/tree/{version}/docs","GitHubAccessToken":"***","GitHubUserAgent":""}', N'/', N'dev', N'')
```
...
- Name: `JELLOG framework (FileSystem)`
...
- ExtraProperties: 
  ...
  ```json
  {"Path":"C:\\Github\\jellog\\docs"}
  ```

  Note that `Path` must be replaced with your local docs directory. You can fetch the JELLOG Framework's documents from https://github.com/jellogframework/jellog/tree/master/docs and copy to the directory `C:\\Github\\jellog\\docs` to get it work.
...
```mssql
INSERT [dbo].[DocsProjects] ([Id], [Name], [ShortName], [Format], [DefaultDocumentName], [NavigationDocumentName], [MinimumVersion], [DocumentStoreType], [ExtraProperties], [MainWebsiteUrl], [LatestVersionBranchName], [ParametersDocumentName]) VALUES (N'12f21123-e08e-4f15-bedb-ae0b2d939659', N'JELLOG framework (FileSystem)', N'jellog', N'md', N'Index', N'docs-nav.json', NULL, N'FileSystem', N'{"Path":"C:\\Github\\jellog\\docs"}', N'/', NULL, N'')
```
...
So far, we have created a new application from jellog.io website and made it up and ready for Docs module. 
...
JELLOG Docs Module can render mark down to HTML. 
...
## This is a sub header
 ...
 [This is a link](https://jellog.io) 

![This is an image](https://jellog.io/assets/my-image.png)
...
As an example you can see JELLOG Framework documentation:

[https://github.com/jellogframework/jellog/blob/master/docs/en/](https://github.com/jellogframework/jellog/blob/master/docs/en/)
...
For example, [en/docs-params.json](https://github.com/jellogio/jellog-commercial-docs/blob/master/en/docs-params.json):
...
For example [Getting-Started.md](https://github.com/jellogio/jellog-commercial-docs/blob/master/en/getting-started.md):
...
{
    options.Enable = true;
    options.IndexName = "your_index_name"; //default IndexName is jellog_documents
});
```
...
Docs Module is also available as a standalone application. Check out [DataGapDocs](../Apps/DataGapDocs).
