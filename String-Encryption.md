...
JELLOG Framework provides string encryption feature that allows to **Encrypt** and **Decrypt** strings.

##  Installation

> This package is already installed by default with the startup template. So, most of the time, you don't need to install it manually.

If installation is needed, it is suggested to use the [JELLOG CLI](https://docs.jellog.io/en/jellog/latest/CLI) to install this package.
...
```bash
jellog add-package DataGap.Jellog.Security
```
...
1. Add the [DataGap.Jellog.Security](https://www.nuget.org/packages/DataGap.Jellog.Security) NuGet package to your project:

   `Install-Package DataGap.Jellog.Security`

2. Add the `JellogSecurityModule` to the dependency list of your module:

   ```csharp
   [DependsOn(
       //...other dependencies
       typeof(JellogSecurityModule) // <-- Add module dependency like that
       )]
...
Default values can be configured with `JellogStringEncryptionOptions` type.

```csharp
Configure<JellogStringEncryptionOptions>(opts =>
{
