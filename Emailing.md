...
JELLOG Framework provides various services, settings and integrations for sending emails;
...
````bash
jellog add-package DataGap.Jellog.Emailing
````

If you haven't done it yet, you first need to install the JELLOG CLI. For other installation options, see [the package description page](https://jellog.io/package-detail/DataGap.Jellog.Emailing).
...
Email sending uses the [setting system](Settings.md) to define settings and get the values of these settings on the runtime. `DataGap.Jellog.Emailing.EmailSettingNames` defines constants for the setting names, just listed below:

* **Jellog.Mailing.DefaultFromAddress**: Used as the sender's email address when you don't specify a sender when sending emails (just like in the example above).
* **Jellog.Mailing.DefaultFromDisplayName**: Used as the sender's display name when you don't specify a sender when sending emails (just like in the example above).
* **Jellog.Mailing.Smtp.Host**: The IP/Domain of the SMTP server (default: 127.0.0.1).
* **Jellog.Mailing.Smtp.Port**: The Port of the SMTP server (default: 25).
* **Jellog.Mailing.Smtp.UserName**: Username, if the SMTP server requires authentication.
* **Jellog.Mailing.Smtp.Password**: Password, if the SMTP server requires authentication. **This value is encrypted **(see the section below).
* **Jellog.Mailing.Smtp.Domain**: Domain for the username, if the SMTP server requires authentication.
* **Jellog.Mailing.Smtp.EnableSsl**: A value that indicates if the SMTP server uses SSL or not ("true" or "false". Default: "false").
* **Jellog.Mailing.Smtp.UseDefaultCredentials**: If true, uses default credentials instead of the provided username and password ("true" or "false". Default: "true").
...
> Setting Management module is already installed if you've created your solution from the JELLOG Startup template.

If you don't use the Setting Management module, you can simply define the settings inside your `appsettings.json` file:

````json
"Settings": {
  "Jellog.Mailing.Smtp.Host": "127.0.0.1",
  "Jellog.Mailing.Smtp.Port": "25",
  "Jellog.Mailing.Smtp.UserName": "",
  "Jellog.Mailing.Smtp.Password": "",
  "Jellog.Mailing.Smtp.Domain": "",
  "Jellog.Mailing.Smtp.EnableSsl": "false",
  "Jellog.Mailing.Smtp.UseDefaultCredentials": "true",
  "Jellog.Mailing.DefaultFromAddress": "noreply@jellog.io",
  "Jellog.Mailing.DefaultFromDisplayName": "JELLOG application"
}
````
...
*Jellog.Mailing.Smtp.Password* must be an **encrypted** value. If you use the `ISettingManager` to set the password, you don't have to worry. It internally encrypts the values on set and decrypts on get.
...
JELLOG Framework provides a strong and flexible [text templating system](Text-Templating.md). You can use the text templating system to create dynamic email contents. Inject the `ITemplateRenderer` and use the `RenderAsync` to render a template. Then use the result as the email body.
...
"**Jellog.StandardEmailTemplates.Message**" is simplest template that has a text message:

````html
{%{{{model.message}}}%}
````

This template uses the "Jellog.StandardEmailTemplates.Layout" as its layout.

"**Jellog.StandardEmailTemplates.Layout**" is a simple template to provide an HTML document layout:
...
> These template names are contants defined in the `DataGap.Jellog.Emailing.Templates.StandardEmailTemplates` class.
...
* `/DataGap/Jellog/Emailing/Templates/Layout.tpl`
* `/DataGap/Jellog/Emailing/Templates/Message.tpl`
