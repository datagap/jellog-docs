...
JELLOG CLI (Command Line Interface) is a command line tool to perform some common operations for JELLOG based solutions.

## Installation

JELLOG CLI is a [dotnet global tool](https://docs.microsoft.com/en-us/dotnet/core/tools/global-tools). Install it using a command line window:
...
* `--skip-cli-version-check`: Skips to check the latest version of the JELLOG CLI. If you don't specify, it will check the latest version and shows a warning message if there is a newer version of the JELLOG CLI.

## Commands

Here, is the list of all available commands before explaining their details:

* **`help`**: Shows help on the usage of the JELLOG CLI.
* **`new`**: Generates a new solution based on the JELLOG [startup templates](Startup-Templates/Index.md).
* **`update`**: Automatically updates all JELLOG related NuGet and NPM packages in a solution.
* **`clean`**: Deletes all `BIN` and `OBJ` folders in the current folder.
* **`add-package`**: Adds an JELLOG package to a project.
* **`add-module`**: Adds a [multi-package application module](https://docs.jellog.io/en/jellog/latest/Modules/Index) to a solution.
...
* **`switch-to-preview`**: Switches to the latest preview version of the JELLOG Framework.
* **`switch-to-nightly`**: Switches to the latest [nightly builds](Nightly-Builds.md) of the JELLOG related packages on a solution.
* **`switch-to-stable`**: Switches to the latest stable versions of the JELLOG related packages on a solution.
...
* **`login`**: Authenticates on your computer with your [jellog.io](https://jellog.io/) username and password.
* **`login-info`**: Shows the current user's login information.
* **`logout`**: Logouts from your computer if you've authenticated before.
* **`bundle`**: Generates script and style references for an JELLOG Blazor project. 
* **`install-libs`**: Install NPM Packages for MVC / Razor Pages and Blazor Server UI types.

### help

Shows basic usages of the JELLOG CLI.

Usage:

````bash
jellog help [command-name]
````

Examples:

````bash
jellog help        # Shows a general help.
jellog help new    # Shows help about the "new" command.
````

### new

Generates a new solution based on the JELLOG [startup templates](Startup-Templates/Index.md).

Usage:

````bash
jellog new <solution-name> [options]
````
...
For more samples, go to [JELLOG CLI Create Solution Samples](CLI-New-Command-Samples.md)
...
* `--version` or `-v`: Specifies the JELLOG & template version. It can be a [release tag](https://github.com/jellogframework/jellog/releases) or a [branch name](https://github.com/jellogframework/jellog/branches). Uses the latest release if not specified. Most of the times, you will want to use the latest version.
...
* `--database-management-system` or `-dbms`: Sets the database management system. Default is **SQL Server**. Supported DBMS's:
  ...
  * `Oracle-Devart`
  * `PostgreSQL`
* `--local-framework-ref --jellog-path`: Uses local projects references to the JELLOG framework instead of using the NuGet packages. This can be useful if you download the JELLOG Framework source code and have a local reference to the framework from your application.
...
Updating all JELLOG related packages can be tedious since there are many packages of the framework and modules. This command automatically updates all JELLOG related NuGet and NPM packages in a solution or project to the latest versions. You can run it in the root folder of your solutions.

Usage:

````bash
jellog update [options]
````

* If you run in a directory with a .csproj file, it updates all JELLOG related packages of the project to the latest versions.
* If you run in a directory with a .sln file, it updates all JELLOG related packages of the all projects of the solution to the latest versions.
...
Adds an JELLOG package to a project by,
...
````bash
jellog add-package <package-name> [options]
````

Example:

````
jellog add-package DataGap.Jellog.AspNetCore.Mvc.UI.Theme.Basic
````

* This example adds the `DataGap.Jellog.AspNetCore.Mvc.UI.Theme.Basic` package to the project.
...
> Currently only the source code of the basic theme packages([MVC](https://docs.jellog.io/en/jellog/latest/UI/AspNetCore/Basic-Theme) and [Blazor](https://docs.jellog.io/en/jellog/latest/UI/Blazor/Basic-Theme)) can be downloaded.
> - DataGap.Jellog.AspNetCore.Mvc.UI.Theme.Basic
> - DataGap.Jellog.AspNetCore.Components.WebAssembly.BasicTheme
> - DataGap.Jellog.AspNetCore.Components.Web.BasicTheme
> - DataGap.Jellog.AspNetCore.Components.Server.BasicTheme
...
````bash
jellog add-module <module-name> [options]
````
...
* This example adds the `DataGap.Blogging` module to the solution.
...
````bash
jellog get-source <module-name> [options]
````

Example:

```bash
jellog get-source DataGap.Blogging

jellog get-source DataGap.Blogging --local-framework-ref --jellog-path D:\GitHub\jellog
```
...
* `--local-framework-ref --jellog-path`: Path of [JELLOG Framework GitHub repository](https://github.com/jellogframework/jellog) in your computer. This will be used for converting project references to your local system. If this is not specified, project references will be converted to NuGet references.
...
````bash
jellog generate-proxy -t <client-type> [options]
````

Examples:

````bash
jellog generate-proxy -t ng -url https://localhost:44302/
jellog generate-proxy -t js -url https://localhost:44302/
jellog generate-proxy -t csharp -url https://localhost:44302/
````
...
````bash
jellog remove-proxy -t <client-type> [options]
````

Examples:

````bash
jellog remove-proxy -t ng
jellog remove-proxy -t js -m identity -o Pages/Identity/client-proxies.js
jellog remove-proxy -t csharp --folder MyProxies/InnerFolder
````
...
You can use this command to switch your project to latest preview version of the JELLOG framework.

Usage:

````bash
jellog switch-to-preview [options]
````
...
You can use this command to switch your project to latest [nightly](Nightly-Builds.md) preview version of the JELLOG framework packages.

Usage:

````bash
jellog switch-to-nightly [options]
````
...
If you're using the JELLOG Framework preview packages (including nightly previews), you can switch back to latest stable version using this command.

Usage:

````bash
jellog switch-to-stable [options]
````
#### Options
...
* It searches all the localization `JSON` files in the current directory and all subdirectories (recursively). Then creates a single file (named `jellog-translation.json` by default) that includes all the entries need to be translated.
...
> The main purpose of this command is to translate JELLOG Framework localization files (since the [jellog repository](https://github.com/jellogframework/jellog) has tens of localization files to be translated in different directories).
...
````bash
jellog translate -c <culture> [options]
````
...
* `--output` or `-o`: Output file name. Default `jellog-translation.json`.
...
````bash
jellog translate --apply  # apply all changes
jellog translate -a       # shortcut for --apply
````

Then review changes on your source control system to be sure that it has changed the proper files and send a Pull Request if you've translated JELLOG Framework resources. Thank you in advance for your contribution.

##### Additional Options

* `--file` or `-f`: Default: `jellog-translation.json`. The translation file (use only if you've used the `--output` option before).

### login

Some features of the CLI requires to be logged in to jellog.io platform. To login with your username write:

```bash
jellog login <username>                                  # Allows you to enter your password hidden
jellog login <username> -p <password>                    # Specify the password as a parameter (password is visible)
jellog login <username> --organization <organization>    # If you have multiple organizations, you need set your active organization
jellog login <username> -p <password> -o <organization>  # You can enter both your password and organization in the same command
```
...
This command generates script and style references for an JELLOG Blazor WebAssembly project and updates the **index.html** file. It helps developers to manage dependencies required by JELLOG modules easily.  In order ```bundle``` command to work, its **executing directory** or passed ```--working-directory``` parameter's directory must contain a Blazor project file(*.csproj).

Usage:

````bash
jellog bundle [options]
````
...
`install-libs` command reads the `jellog.resourcemapping.js` file to manage package. For more details see [Client Side Package Management](UI/AspNetCore/Client-Side-Package-Management.md).

Usage:

````bash
jellog install-libs [options]
````
