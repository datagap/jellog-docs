...
[DataGap.Jellog.Data](https://www.nuget.org/packages/DataGap.Jellog.Data) package defines services to automatically filter data on querying from a database.

## Pre-Defined Filters

JELLOG defines some filters out of the box.
...
## JellogDataFilterOptions

`JellogDataFilterOptions` can be used to [set options](Options.md) for the data filter system.
...
````csharp
Configure<JellogDataFilterOptions>(options =>
{
...
Defining and implementing a new filter highly depends on the database provider. JELLOG implements all pre-defined filters for all database providers.
...
JELLOG uses [EF Core's Global Query Filters](https://docs.microsoft.com/en-us/ef/core/querying/filters) system for the [EF Core Integration](Entity-Framework-Core.md). So, it is well integrated to EF Core and works as expected even if you directly work with `DbContext`.
...
JELLOG abstracts the `IMongoDbRepositoryFilterer` interface to implement data filtering for the [MongoDB Integration](MongoDB.md), it works only if you use the repositories properly. Otherwise, you should manually filter the data.
...
public abstract class MyMongoRepository<TMongoDbContext, TEntity, TKey> : MongoDbRepository<TMongoDbContext, TEntity, TKey>
    where TMongoDbContext : IJellogMongoDbContext
...
public class MyMongoDbModule : JellogModule
{
